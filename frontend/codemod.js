...
function getInferredType(paramName) {
    const name = paramName.toLowerCase();
    if (name === "id" || name.endsWith("id"))
        return "string | number";
    if (name.startsWith("is") || name.startsWith("has") || name.startsWith("should"))
        return "boolean";
    if (name === "e" || name === "event")
        return "any"; // Avoid 'any', but SyntheticEvent is hard to auto-import safely, we'll use unknown later if needed
...
function inferTypeForBindingPattern(pattern) {
        ...
        callable.getParameters().forEach(param => {
            if (!param.getTypeNode()) {
                ...
                if (nameNode.getKind() === SyntaxKind.ObjectBindingPattern) {
                    const typeStr = inferTypeForBindingPattern(nameNode);
