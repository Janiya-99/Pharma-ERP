...
const FieldWrap: React.FC<{
  ...
  <div className="flex flex-col gap-1.5">
    <Label className="text-[11px] font-bold uppercase tracking-widest text-[#052659]">
      {label}
    ...
    {hint && !error && (
      <span className="text-[11px] leading-relaxed text-[#7DA0CA]">{hint}</span>
    )}
  </div>
);
...
const inp = (err?: string) =>
  `h-10 border bg-white/70 text-[#021024] placeholder:text-[#7DA0CA]/70 rounded-xl
   ...
   focus-visible:border-[#052659] ${
     err ? "border-red-400" : "border-[#7DA0CA]"
   }`;

const selectTrigCls = `h-10 w-full min-w-0 rounded-xl border-[#7DA0CA] bg-white px-3
...
const selectContentCls = `z-[100] max-h-72 w-[var(--radix-select-trigger-width)] min-w-[var(--radix-select-trigger-width)]
   overflow-y-auto rounded-xl border border-[#7DA0CA]/35 bg-white p-1.5
   shadow-[0_18px_48px_rgba(5,38,89,0.18)]`;

const selectItemCls = `rounded-lg px-2.5 py-2 text-sm text-[#052659]
...
const BrandToggle: React.FC<{
  ...
  <div className="flex items-center justify-between gap-4 px-4 py-3.5">
    <div className="min-w-0">
      <p className="text-sm font-semibold leading-snug text-[#021024]">
        {label}
      </p>
      {description && (
        <p className="mt-0.5 text-xs leading-relaxed text-[#7DA0CA]">
          {description}
        </p>
      )}
    </div>
    <button
      ...
      className={`relative inline-flex h-[26px] w-12 flex-shrink-0 items-center rounded-full
        ...
        ${
          checked
            ...
            : "border border-[#7DA0CA] bg-[#C1E8FF]"
        }`}
    >
      <span
        className={`inline-block h-[18px] w-[18px] transform rounded-full bg-white shadow-md
...
const SectionHeader: React.FC<{
  ...
  <div
    className="-mx-6 mb-5 flex items-center gap-3 border-b border-[#C1E8FF]/60 px-6 py-4"
    style={{
      background: `linear-gradient(135deg, ${color}12 0%, transparent 60%)`,
    }}
  >
    <div
      className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl text-xs font-black text-white shadow-lg"
      style={{
        background: `linear-gradient(135deg, ${color} 0%, #021024 120%)`,
      }}
    >
      {step}
    </div>
    <div className="flex flex-1 items-center gap-2">
      ...
      <h3 className="text-sm font-bold tracking-tight text-[#021024]">
        {title}
      </h3>
    </div>
  </div>
...
const GlassCard: React.FC<{
  ...
  <div
    className={`relative overflow-hidden rounded-2xl border border-[#7DA0CA]/40
      bg-white/80 shadow-[0_4px_32px_rgba(5,38,89,0.10)] backdrop-blur-sm 
      transition-shadow duration-300 hover:shadow-[0_8px_40px_rgba(5,38,89,0.16)] ${className}`}
  >
    <div
      className="absolute bottom-0 left-0 top-0 w-1 rounded-l-2xl"
      style={{
        background: `linear-gradient(180deg, ${accent} 0%, #7DA0CA 100%)`,
      }}
    />
    {children}
  </div>
...
const UserCreatePage: React.FC = () => {
  ...
  const fetchDropdowns = async () => {
    ...
    if (dsR.status === "fulfilled")
      setDesignations(listFromResponse(dsR.value));
    ...
    try {
      ...
      setForm((p) => ({
        ...
        default_branch_id: u.default_branch_id
          ? String(u.default_branch_id)
          : "",
        user_type: u.user_type || "company_user",
  ...
  const payload = () => {
    ...
    return {
      ...
      user_type: form.user_type || "company_user",
      status: form.status,
  ...
  const accessManagementPath = (userId?: string | number) =>
    userId
      ? `/control-center/user-access?userId=${encodeURIComponent(
          String(userId)
        )}`
  ...
  const handleResetPwd = async () => {
    if (resetPwd.new_password.length < 8) {
      toast.error("Min 8 chars");
      return;
    }
    if (resetPwd.new_password !== resetPwd.confirm_password) {
    ...
    try {
      ...
      if (res.success) {
        ...
        setResetPwd({ new_password: "", confirm_password: "" });
    ...
    return (
      <div className="relative isolate min-h-screen bg-white">
        <div className="page-content space-y-6">
          {[1, 2, 3].map((i) => (
            <Skeleton key={i} className="h-48 w-full rounded-2xl bg-white/50" />
          ))}
        </div>
      </div>
    );
      ...
      <div className="relative isolate min-h-screen bg-white">
        <div className="page-content">
          <Alert variant="destructive">
            <AlertDescription>
              Unable to load user.{" "}
              <button className="font-semibold underline" onClick={fetchUser}>
                Retry
              </button>
            </AlertDescription>
          </Alert>
      ...
      <div className="page-content space-y-6 pb-10">
        ...
        <div className="flex items-start gap-3">
          <button
            ...
            className="mt-0.5 rounded-xl border border-[#7DA0CA]/50 bg-white/70 p-2.5 text-[#052659]
              shadow-sm backdrop-blur-sm transition-all duration-200 hover:bg-white hover:shadow-md"
          >
        ...
        <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-3">
                ...
                <div className="grid grid-cols-1 gap-4 px-6 pb-6 sm:grid-cols-2">
                  ...
                  <FieldWrap label="Employee Code">
                    <Input
                      ...
                      onChange={(e) => sf("employee_code", e.target.value)}
                ...
                <div className="grid grid-cols-1 gap-4 px-6 pb-6 sm:grid-cols-2">
                  <FieldWrap label="Department">
                    ...
                    >
                      ...
                      <SelectContent align="start" className={selectContentCls}>
                        <SelectItem value="__none__" className={selectItemCls}>
                          — No department —
                        </SelectItem>
                  ...
                  <FieldWrap label="Designation">
                    <Select
                      ...
                      onValueChange={(v) =>
                        sf("designation_id", v === "__none__" ? "" : v)
                      }
                    >
                      <SelectTrigger className={selectTrigCls}>
                  ...
                  <FieldWrap
                    ...
                    hint="Home branch — full access set in Access Management"
                  >
                    <Select
                      value={form.default_branch_id || "__none__"}
                      onValueChange={(v) =>
                        sf("default_branch_id", v === "__none__" ? "" : v)
                      }
                    >
                      <SelectTrigger className={selectTrigCls}>
                  ...
                  <FieldWrap label="Status" required error={errors.status}>
                    ...
                    >
                      ...
                      <SelectContent align="start" className={selectContentCls}>
                        ...
                        <SelectItem value="inactive" className={selectItemCls}>
                          ⚪ Inactive
                        </SelectItem>
                        <SelectItem value="suspended" className={selectItemCls}>
                          ⚠️ Suspended
                        </SelectItem>
                        <SelectItem value="locked" className={selectItemCls}>
                          🔒 Locked
                        </SelectItem>
                      </SelectContent>
                    </Select>
            ...
            <GlassCard accent="#7DA0CA">
              <div className="pl-3">
                ...
                <div className="space-y-5 px-6 pb-6">
                  {/* Enable login row */}
                  <div
                    className="from-[#052659]/8 flex items-center justify-between rounded-2xl
                    border border-[#7DA0CA]/40 bg-gradient-to-r
                    to-[#C1E8FF]/40 p-4 backdrop-blur-sm"
                  >
                    <div>
                      <p className="text-sm font-bold text-[#021024]">
                        Enable Login Access
                      </p>
                      <p className="mt-0.5 text-xs text-[#5483B3]">
                    ...
                    <button
                      type="button"
                      role="switch"
                      aria-checked={form.login_enabled}
                    ...
                    >
                      <span
                        className={`inline-block h-[18px] w-[18px] transform rounded-full bg-white
                  ...
                  {!isEdit && form.login_enabled && (
                    <div
                      className="grid grid-cols-1 gap-4 rounded-2xl border border-[#7DA0CA]/40
                      bg-[#C1E8FF]/30 p-4 sm:grid-cols-2"
                    >
                      <FieldWrap
                      ...
                      >
                        <div className="relative">
                          <Input
                            ...
                            className={`pr-10 ${inp(errors.password)}`}
                          />
                          <button
                            type="button"
                            onClick={() => setShowPwd(!showPwd)}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7DA0CA] transition-colors hover:text-[#052659]"
                          >
                            {showPwd ? (
                      ...
                      >
                        <div className="relative">
                          <Input
                            ...
                            onChange={(e) =>
                              sf("confirm_password", e.target.value)
                            }
                            placeholder="Re-enter password"
                            className={`pr-10 ${inp(errors.confirm_password)}`}
                          />
                          <button
                            type="button"
                    ...
                    <div>
                      {!showResetForm ? (
                        <button
                          ...
                          className="flex items-center gap-2 rounded-xl border border-[#7DA0CA] bg-white/60
                            px-4 py-2 text-sm font-semibold text-[#052659]
                            transition-all hover:bg-[#C1E8FF]/60 hover:shadow-sm"
                        >
                          <Lock className="h-3.5 w-3.5" /> Reset Password
                        </button>
                      ) : (
                        <div className="space-y-4 rounded-2xl border border-[#7DA0CA]/50 bg-[#C1E8FF]/30 p-4">
                          ...
                          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <FieldWrap label="Confirm Password" required>
                              <Input
                                type="password"
                                value={resetPwd.confirm_password}
                                onChange={(e) =>
                                  setResetPwd((p) => ({
                                    ...p,
                          ...
                          <div className="flex gap-2 pt-1">
                            <button
                              ...
                              className="rounded-xl bg-gradient-to-r from-[#052659] to-[#5483B3] px-5 py-2
                            ...
                            <button
                              ...
                              className="rounded-xl px-4 py-2 text-sm font-medium text-[#5483B3] transition-colors hover:text-[#052659]"
                            >
                              Cancel
                            </button>
                          </div>
                  ...
                  {/* Security toggles */}
                  <div className="divide-y divide-[#C1E8FF] overflow-hidden rounded-2xl border border-[#C1E8FF] bg-white/50">
                    <BrandToggle
                      id="force_pwd"
          ...
          <div className="space-y-4">
            {/* How-to steps */}
            <div
              className="overflow-hidden rounded-2xl border border-[#7DA0CA]/40 bg-white/70
              shadow-[0_4px_24px_rgba(5,38,89,0.08)] backdrop-blur-sm"
            >
              <div className="border-b border-[#C1E8FF] px-5 pb-3 pt-5">
                <div className="flex items-center gap-2">
                  <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#C1E8FF]">
              ...
              <div className="space-y-3 p-5">
                {[
                  { n: 1, c: "#052659", t: "Fill in name, email & contact" },
                  {
                    n: 2,
                    c: "#5483B3",
                    t: "Set department, designation & branch",
                  },
                  ...
                  { n: 4, c: "#021024", t: "Save the user profile" },
                  {
                    n: 5,
                    c: "#052659",
                    t: "Assign access in Access Management",
                  },
                ].map(({ n, c, t }) => (
                  <div key={n} className="flex items-start gap-3">
                    <div
                      className="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-[10px] font-black text-white"
                      style={{ background: c }}
                    >
                      {n}
                    </div>
                    <p className="pt-0.5 text-xs leading-relaxed text-[#5483B3]">
                      {t}
                    </p>
                  </div>
                ))}
            ...
            <div className="overflow-hidden rounded-2xl shadow-[0_4px_24px_rgba(5,38,89,0.14)]">
              <div
                className="p-5"
                style={{
                  background:
                    "linear-gradient(135deg, #021024 0%, #052659 60%, #5483B3 120%)",
                }}
              >
                ...
                <p className="text-xs leading-relaxed text-[#7DA0CA]">
                  After saving the user, assign branches, software modules,
                  roles, and permissions from Access Management.
                </p>
              </div>
              <div className="bg-white/90 px-5 py-3.5 backdrop-blur-sm">
                <button
                  type="button"
                  onClick={() => navigate(accessManagementPath(id))}
                  className="w-full rounded-xl border-2 border-[#052659]/20 bg-[#C1E8FF]/50 py-2.5
                    text-sm font-bold text-[#052659]
                    transition-all duration-200 hover:border-[#052659]/40 hover:bg-[#C1E8FF]"
                >
            ...
            {/* Status legend */}
            <div
              className="rounded-2xl border border-[#7DA0CA]/40 bg-white/70 p-5
              shadow-[0_4px_24px_rgba(5,38,89,0.08)] backdrop-blur-sm"
            >
              <p className="mb-4 text-sm font-bold text-[#021024]">
                Status Guide
              </p>
                ...
                {[
                  ...
                  <div key={l} className="flex items-center gap-3">
                    <div
                      ...
                      style={{ background: c }}
                    />
                    <span className="w-16 text-xs font-bold text-[#052659]">
                      {l}
                    </span>
        ...
        {/* Action bar */}
        <div
          className="border-slate-200 rounded-2xl border bg-white px-5 py-4
        ...
        >
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <button
              ...
              disabled={saving}
              onClick={() => navigate("/control-center/users")}
              className="text-slate-500 hover:bg-slate-100 hover:text-slate-900 inline-flex items-center justify-center gap-2
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              {!isEdit && (
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#7DA0CA]/70
                    ...
                    transition-all hover:-translate-y-0.5 hover:border-[#5483B3]
                    hover:bg-[#f7fbff] hover:shadow-md disabled:translate-y-0 disabled:cursor-not-allowed
                    disabled:opacity-60"
                >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#052659]/25
                  bg-white px-5 py-2.5 text-sm font-bold text-[#052659] shadow-sm
                  transition-all hover:-translate-y-0.5 hover:border-[#052659]/50
                  hover:bg-[#C1E8FF]/50 hover:shadow-md disabled:translate-y-0 disabled:cursor-not-allowed
                  disabled:opacity-60"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#052659]
                  ...
                  transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#021024]
                  hover:shadow-xl hover:shadow-[#052659]/30 focus-visible:outline-none
                  focus-visible:ring-2 focus-visible:ring-[#5483B3]/45 focus-visible:ring-offset-2 active:translate-y-0
