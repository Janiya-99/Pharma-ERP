...
const LoginLogsPage = () => {
  ...
  const currentUsers = useMemo(
    () =>
      new Set(
        logs.map(
          (row: any) =>
            row.user?.full_name || row.user_name || row.email || "System"
        )
      ).size,
    [logs]
  );
  const successCount = useMemo(
    () => logs.filter((row: any) => row.status === "success").length,
    [logs]
  );
  const failureCount = useMemo(
    () => logs.filter((row: any) => row.status === "failed").length,
    [logs]
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: "ip_address",
      cell: (row: any) => (
        <span className="text-slate-500 bg-slate-50 rounded-lg px-2 py-1 font-mono text-xs">
          {row.ip_address || "-"}
        </span>
      ),
    },
    {
      ...
      cell: (row: any) => (
        <span
          className="text-slate-500 block max-w-[220px] truncate text-xs"
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-slate-500 text-xs">
                Authentication attempts on the current result set.
              </p>
            </div>
