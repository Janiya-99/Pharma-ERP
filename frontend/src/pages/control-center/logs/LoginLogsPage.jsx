...
const LoginLogsPage = () => {
  ...
  const columns = [
    ...
    },
    {
      header: "User",
      accessor: "user_name",
      cell: (row) => row.user?.full_name || row.user_name || "-"
    },
    {
      header: "Email",
      accessor: "email",
    ...
    },
    {
      ...
      cell: (row) => <span className="text-red-600 text-xs">{row.failure_reason || "-"}</span>
    },
    {
      header: "IP Address",
      accessor: "ip_address",
      cell: (row) => <span className="font-mono text-xs text-gray-500 bg-gray-50 px-1 rounded">{row.ip_address || "-"}</span>
    },
    {
      header: "User Agent",
      accessor: "user_agent",
