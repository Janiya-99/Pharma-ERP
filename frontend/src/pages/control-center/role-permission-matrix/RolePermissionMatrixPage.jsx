...
const RolePermissionMatrixPage = () => {
  ...
  useEffect(() => {
    const isDifferent = 
      ...
      !assignedPermissionIds.every(id => initialAssignedIds.includes(id)) ||
      !initialAssignedIds.every(id => assignedPermissionIds.includes(id));
      
    setHasUnsavedChanges(isDifferent);
  }, [assignedPermissionIds, initialAssignedIds]);

  const fetchGroupedPermissions = async (softwareId) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const handleGroupSelectionChange = (groupIds, groupName) => {
    const groupPermIds = groupedPermissions
      .find(g => g.permission_group === groupName)
      ?.permissions.map(p => p.id) || [];
      
    // Remove old ids for this group
    let newSelection = assignedPermissionIds.filter(id => !groupPermIds.includes(id));
    // Add new ones
      ...
      {!selectedSoftware ? (
        ...
        <div className="space-y-6 pb-20">
          ...
          {groupedPermissions.map((group, idx) => {
            ...
            const selectedInGroup = assignedPermissionIds.filter(id => groupPermIds.includes(id));
            ...
            return (
              <PermissionCheckboxGroup
                ...
                onSelectionChange={(newSelectedInGroup) => 
