...
const StockTransferLinesTable = ({ lines, setLines, sourceWarehouseId, destinationWarehouseId, errors = {} }) => {
  ...
  const fetchSourceLocations = async (wId) => {
    try {
  ...
  const fetchDestinationLocations = async (wId) => {
    try {
      const res = await inventoryApi.getWarehouseLocations({ warehouse_id: wId, limit: 1000, status: "active" });
      if (res.success !== false) {
  ...
  const fetchAvailableStock = async (index, productId, batchId, locationId) => {
                  ...
                  <tr key={line.id || index} className="hover:bg-gray-50 dark:hover:bg-navy-800/50">
                    ...
                    <td className="px-4 py-3 align-top">
                      <select
                        ...
                        onChange={(e) => handleLineChange(index, "source_location_id", e.target.value ? parseInt(e.target.value) : null)}
                    ...
                    <td className="px-4 py-3 align-top">
                      <select
                        ...
                        onChange={(e) => handleLineChange(index, "destination_location_id", e.target.value ? parseInt(e.target.value) : null)}
                    ...
                    <td className="px-4 py-3 align-top">
                      <TransferQuantityInput
                        ...
                        onChange={(val) => handleLineChange(index, "transfer_quantity", val)}
