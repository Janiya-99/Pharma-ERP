...
import GenericNameFormModal from "./GenericNameFormModal";
...
const GenericNamesPage = () => {
  ...
  const handleDelete = async (row: unknown) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await inventoryApi.deleteGenericName(row.id);
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy-700 ">Generic Names</h1>
...
export default GenericNamesPage;
