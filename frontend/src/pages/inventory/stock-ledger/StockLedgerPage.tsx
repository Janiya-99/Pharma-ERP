  ...
  const columns = [
    {
      ...
      cell: ({ row }: { row?: unknown }) =>
        new Date(row.original.transaction_date).toLocaleString(),
    },
    {
      header: "Source",
    ...
    { header: "Warehouse", accessorKey: "warehouse.warehouse_name" },
    {
      ...
      cell: ({ row }: { row?: unknown }) =>
        row.original.quantity_in > 0 ? (
          <StockQuantityDisplay quantity={row.original.quantity_in} />
        ) : (
          "-"
        ),
    },
    {
      ...
      cell: ({ row }: { row?: unknown }) =>
        row.original.quantity_out > 0 ? (
          <StockQuantityDisplay quantity={row.original.quantity_out} />
        ) : (
          "-"
        ),
    },
    {
      ...
      cell: ({ row }: { row?: unknown }) => (
        <StockQuantityDisplay quantity={row.original.balance_quantity} />
      ),
    },
    {
      header: "Unit Cost",
      ...
      cell: ({ row }: { row?: unknown }) => (
        <StockValueDisplay value={row.original.unit_cost} />
      ),
    },
    {
      header: "Total Cost",
      ...
      cell: ({ row }: { row?: unknown }) => (
        <StockValueDisplay value={row.original.total_cost} />
      ),
    },
  ];
