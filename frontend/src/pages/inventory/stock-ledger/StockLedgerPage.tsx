...
const StockLedgerPage = () => {
  ...
  const columns = [
    { header: "Date", accessorKey: "transaction_date", cell: ({ row }: { row?: unknown }) => new Date(row.original.transaction_date).toLocaleString() },
    { header: "Source", accessorKey: "source_type", cell: ({ row }: { row?: unknown }) => <span className="uppercase text-xs font-semibold">{row.original.source_type}</span> },
    ...
    { header: "Qty In", accessorKey: "quantity_in", cell: ({ row }: { row?: unknown }) => row.original.quantity_in > 0 ? <StockQuantityDisplay quantity={row.original.quantity_in} /> : "-" },
    { header: "Qty Out", accessorKey: "quantity_out", cell: ({ row }: { row?: unknown }) => row.original.quantity_out > 0 ? <StockQuantityDisplay quantity={row.original.quantity_out} /> : "-" },
    { header: "Balance Qty", accessorKey: "balance_quantity", cell: ({ row }: { row?: unknown }) => <StockQuantityDisplay quantity={row.original.balance_quantity} /> },
    { header: "Unit Cost", accessorKey: "unit_cost", cell: ({ row }: { row?: unknown }) => <StockValueDisplay value={row.original.unit_cost} /> },
    { header: "Total Cost", accessorKey: "total_cost", cell: ({ row }: { row?: unknown }) => <StockValueDisplay value={row.original.total_cost} /> },
  ];
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy-700 ">Stock Ledger</h1>
