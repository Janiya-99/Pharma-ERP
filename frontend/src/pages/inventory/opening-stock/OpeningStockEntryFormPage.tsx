...
const OpeningStockEntryFormPage = () => {
      ...
      if (data) {
        setFormData({
          ...
          opening_stock_date: data.opening_stock_date
            ? data.opening_stock_date.split("T")[0]
            : "",
  ...
  const validateForm = () => {
    ...
    if (!formData.opening_stock_date)
      newErrors.opening_stock_date = "Date is required";
    ...
    lines.forEach((line: any, index: number) => {
      ...
      if (line.product?.requires_batch_tracking && !line.product_batch_id) {
        newErrors[`lines.${index}`] = {
          ...newErrors[`lines.${index}`],
          product_batch_id: "Batch is required",
        };
      }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: any) =>
                    setFormData({
                      ...formData,
                      opening_stock_date: e.target.value,
                    })
                  }
                  className={`w-full rounded-xl border bg-white px-4 py-2 text-sm text-gray-900 focus:ring-2 focus:ring-brand-500 dark:bg-navy-900 dark:text-white ${
                    errors.opening_stock_date
                      ? "border-red-500"
                      : "border-gray-200 dark:border-navy-600"
                  }`}
                />
              ...
              <div>
                ...
                {financialYears.length === 0 && (
                  <p className="mt-1 text-xs text-amber-500">
                    Lookup unavailable in current scope
                  </p>
                )}
              </div>
