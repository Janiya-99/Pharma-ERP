...
const ManufacturersPage = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await inventoryApi.getManufacturers({
  ...
  const handleDelete = async (row: unknown) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await inventoryApi.deleteManufacturer(row.id);
  ...
  const columns = [
    {"header": "Code", "accessorKey": "manufacturer_code"},
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy-700 dark:text-white">Manufacturers</h1>
          <p className="text-sm text-gray-500 mt-1">Manage manufacturers</p>
        </div>
