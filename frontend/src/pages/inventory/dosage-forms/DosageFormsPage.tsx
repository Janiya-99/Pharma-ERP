...
import DosageFormFormModal from "./DosageFormFormModal";
...
const DosageFormsPage = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await inventoryApi.getDosageForms({
  ...
  const handleDelete = async (row: unknown) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await inventoryApi.deleteDosageForm(row.id);
  ...
  const columns = [
    { header: "Dosage Form Code", accessorKey: "dosage_form_code" },
    { header: "Dosage Form Name", accessorKey: "dosage_form_name" },
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage dosage forms</p>
        </div>
...
export default DosageFormsPage;
