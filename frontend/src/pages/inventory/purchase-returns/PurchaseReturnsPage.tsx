  ...
  const columns = useMemo(() => [
    {
      ...
      cell: ({ row }: { row: { original: PurchaseReturn } }) => (
        <span className="font-medium text-blue-600">{row.original.purchase_return_number}</span>
      ),
    },
    {
      header: "Date",
      ...
      cell: ({ row }: { row: { original: PurchaseReturn } }) => (
        <span>{new Date(row.original.purchase_return_date).toLocaleDateString()}</span>
      ),
    },
    {
      header: "Supplier",
      accessorKey: "supplier.supplier_name",
      cell: ({ row }: { row: { original: PurchaseReturn } }) => row.original.supplier?.supplier_name || "N/A",
    },
    {
      header: "Warehouse",
      accessorKey: "warehouse.warehouse_name",
      cell: ({ row }: { row: { original: PurchaseReturn } }) => row.original.warehouse?.warehouse_name || "N/A",
    },
    {
      header: "Reason",
      ...
      cell: ({ row }: { row: { original: PurchaseReturn } }) => (
        <div className="flex flex-col gap-1">
          ...
          {row.original.posted_status === "posted" && (
            <PurchaseReturnPostedStatusBadge status={row.original.posted_status} />
          )}
        </div>
      ),
    },
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => navigate(`/inventory/purchase-returns/${row.original.id}`)}>
                <Eye className="mr-2 h-4 w-4" /> View Details
              </DropdownMenuItem>
              {canEditOrDelete && (
                <PermissionGuard permission="inventory.purchase_return.update">
                  <DropdownMenuItem onClick={() => navigate(`/inventory/purchase-returns/${row.original.id}/edit`)}>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-semibold text-slate-800">Purchase Returns</h1>
          <p className="text-sm text-slate-500">Manage returns to suppliers</p>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
          ...
          <DataTable
            ...
            data={data}
            loading={loading}
            onRowClick={(row: PurchaseReturn) => navigate(`/inventory/purchase-returns/${row.id}`)}
            pagination={{
              page,
              limit,
              total,
