...
const PurchaseReturnDetailsPage: React.FC = () => {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(
        "Failed to post purchase return. It may exceed available stock or have already been posted."
      );
    } finally {
      setActionLoading(false);
    }
          ...
          <div>
            <h1 className="text-slate-800 flex items-center gap-3 text-2xl font-semibold">
              ...
              {data.posted_status === "posted" && (
                <PurchaseReturnPostedStatusBadge status={data.posted_status} />
              )}
            </h1>
            <p className="text-slate-500 text-sm">
              Created on{" "}
              {new Date(data.purchase_return_date).toLocaleDateString()}
            </p>
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm md:grid-cols-3">
                <div>
                  <span className="mb-1 block text-gray-500">Supplier</span>
                  ...
                  <tbody className="divide-y">
                    {data.lines?.map((line: any, index: number) => (
                      <tr key={index} className="bg-white hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-right">
                          {Number(line.discount_amount).toLocaleString()}
                        </td>
                        <td className="px-4 py-3 text-right">
