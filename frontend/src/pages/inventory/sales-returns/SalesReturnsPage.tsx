  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: SalesReturn } }) => (
          <span>
            {new Date(row.original.sales_return_date).toLocaleDateString()}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: SalesReturn } }) => (
          <SalesReturnConditionBadge
            condition={row.original.return_condition}
          />
        ),
      },
      {
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() =>
                    navigate(`/inventory/sales-returns/${row.original.id}`)
                  }
                >
                ...
                {canEditOrDelete && (
                  <PermissionGuard permission="inventory.sales_return.update">
                    <DropdownMenuItem
                      onClick={() =>
                        navigate(
                          `/inventory/sales-returns/${row.original.id}/edit`
                        )
                      }
                    >
      ...
      <Card>
        <CardContent className="p-0">
          ...
          <DataTable
            ...
            data={data}
            loading={loading}
            onRowClick={(row: SalesReturn) =>
              navigate(`/inventory/sales-returns/${row.id}`)
            }
            pagination={{
              page,
              limit,
