...
const StockAdjustmentsPage = () => {
  ...
  const openSubmitModal = (adjustment) => {
    setSelectedAdjustment(adjustment);
    setIsSubmitModalOpen(true);
  };

  const openApproveModal = (adjustment) => {
    setSelectedAdjustment(adjustment);
  ...
  const openPostModal = (adjustment) => {
    setSelectedAdjustment(adjustment);
  ...
  const columns = [
    {
      name: "Date",
      selector: (row) => formatDate(row.adjustment_date),
      sortable: true,
      width: "120px",
    },
    {
      name: "Adjustment No",
      selector: (row) => row.adjustment_number,
      sortable: true,
      cell: (row) => (
        <Link
    ...
    {
      name: "Warehouse",
      selector: (row) => row.warehouse?.warehouse_name || "N/A",
      sortable: true,
    },
    {
      name: "Type",
      selector: (row) => row.adjustment_type,
      cell: (row) => <StockAdjustmentTypeBadge type={row.adjustment_type} />,
      width: "140px",
    },
    {
      name: "Status",
      selector: (row) => row.approval_status,
      cell: (row) => <StockAdjustmentStatusBadge status={row.approval_status} />,
      width: "130px",
    },
    {
      name: "Posting",
      selector: (row) => row.posted_status,
      cell: (row) => <StockAdjustmentPostedStatusBadge status={row.posted_status} />,
      width: "130px",
    },
    {
      name: "Actions",
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      ...
      <div className="bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden mb-6">
        ...
        <DataTable
          ...
          onRowClicked={(row) => navigate(`/inventory/stock-adjustments/${row.id}`)}
          pointerOnHover
          highlightOnHover
        />
      </div>
