...
const StockAdjustmentLinesTable = ({
      ...
      prevLines.map((line) => {
        ...
        if (field === "product_batch_id") {
          if (updatedLine.product?.batches) {
            const batch = updatedLine.product.batches.find((b) => b.id === value);
  ...
  const fetchStockBalanceForLine = async (lineId, productId, batchId, locationId) => {
    if (!warehouseId || !productId || !locationId) return;

    setLines((prev) => prev.map((l) => (l.id === lineId ? { ...l, stock_balance_loading: true } : l)));
    ...
    try {
      ...
      setLines((prev) => prev.map((l) => (l.id === lineId ? { ...l, stock_balance_loading: false } : l)));
    }
  };
                  ...
                  <tr key={line.id} className="bg-white dark:bg-navy-900 group">
                    ...
                    <td className="px-4 py-3 align-top">
                      <StockAdjustmentLineProductSelect
                        ...
                        onChange={(p) => updateLine(line.id, "product_id", p.id)}
                        products={products}
                        error={productError}
                      />
                    ...
                    <td className="px-4 py-3 align-top">
                      <StockAdjustmentLineBatchSelect
                        ...
                        onChange={(b) => updateLine(line.id, "product_batch_id", b.id)}
                    ...
                    <td className="px-4 py-3 align-top">
                      <WarehouseLocationSelect
                        ...
                        onChange={(e) => updateLine(line.id, "warehouse_location_id", e.target.value)}
                      ...
                      <>
                        ...
                        <td className="px-4 py-3 align-top">
                          <input
                            ...
                            onChange={(e) => updateLine(line.id, "physical_quantity", e.target.value)}
                            className={`w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-brand-500 bg-white dark:bg-navy-900 text-gray-900 dark:text-white ${
