...
const StockAdjustmentsPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: StockAdjustment } }) => formatDate(row.original.adjustment_date),
    },
    {
      header: "Branch",
      accessorKey: "branch.branch_name",
      cell: ({ row }: { row: { original: StockAdjustment } }) => row.original.branch?.branch_name || "—",
    },
    {
      header: "Warehouse",
      accessorKey: "warehouse.warehouse_name",
      cell: ({ row }: { row: { original: StockAdjustment } }) => row.original.warehouse?.warehouse_name || "—",
    },
    {
      header: "Reference No",
      accessorKey: "reference_number",
      cell: ({ row }: { row: { original: StockAdjustment } }) => row.original.reference_number || row.original.reference_no || "—",
    },
    {
      header: "Type",
      accessorKey: "adjustment_type",
      cell: ({ row }: { row: { original: StockAdjustment } }) => <StockAdjustmentTypeBadge type={row.original.adjustment_type} />,
    },
    {
      header: "Qty In",
      accessorKey: "total_quantity_in",
      cell: ({ row }: { row: { original: StockAdjustment } }) => formatNumber(row.original.total_quantity_in || 0, 3),
    },
    {
      header: "Qty Out",
      accessorKey: "total_quantity_out",
      cell: ({ row }: { row: { original: StockAdjustment } }) => formatNumber(row.original.total_quantity_out || 0, 3),
    },
    {
      header: "Total Value",
      accessorKey: "total_stock_value",
      cell: ({ row }: { row: { original: StockAdjustment } }) => formatCurrency(row.original.total_stock_value || 0),
    },
    {
      header: "Status",
      accessorKey: "approval_status",
      cell: ({ row }: { row: { original: StockAdjustment } }) => <StockAdjustmentStatusBadge status={row.original.approval_status} />,
    },
    {
      header: "Posting",
      accessorKey: "posted_status",
      cell: ({ row }: { row: { original: StockAdjustment } }) => <StockAdjustmentPostedStatusBadge status={row.original.posted_status} />,
    },
    {
      header: "Created By",
      accessorKey: "created_by_user.name",
      cell: ({ row }: { row: { original: StockAdjustment } }) => row.original.created_by_user?.name || "—",
    },
    {
      header: "Created At",
      accessorKey: "created_at",
      cell: ({ row }: { row: { original: StockAdjustment } }) => formatDate(row.original.created_at || ""),
    },
    {
      header: "Actions",
      cell: ({ row }: { row: { original: StockAdjustment } }) => (
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      ...
      <div className="bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden mb-6">
        ...
        <DataTable
          ...
          data={adjustments}
          loading={loading}
          onRowClick={(row: StockAdjustment) => navigate(`/inventory/stock-adjustments/${row.id}`)}
