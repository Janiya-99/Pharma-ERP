    ...
    <div className="p-6 max-w-[1600px] mx-auto pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white  rounded-xl shadow-sm border border-gray-100  p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500  mb-1">Adjustment Type</h3>
            ...
            <div className="bg-white  rounded-xl shadow-sm border border-gray-100  p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500  mb-1">Adjustment Date</h3>
          ...
          <div className="bg-white  rounded-xl shadow-sm border border-gray-100  p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {adjustment.reference_number || adjustment.reference_no ? (
                <div>
                  <span className="text-sm font-medium text-gray-500  block mb-1">Reference Number</span>
                  <p className="text-sm text-gray-900  font-medium">{adjustment.reference_number || adjustment.reference_no}</p>
                </div>
              ) : null}

              {adjustment.reason && (
                <div>
                  <span className="text-sm font-medium text-gray-500  block mb-1">Adjustment Reason</span>
                  <p className="text-sm text-gray-900  font-medium">{adjustment.reason}</p>
                </div>
              )}
              ...
              {adjustment.created_by_user?.name && (
                <div>
                  ...
                  <p className="text-sm text-gray-900 ">{adjustment.created_by_user.name}</p>
                </div>
              )}

              {adjustment.approved_by_user?.name && (
                <div>
                  <span className="text-sm font-medium text-gray-500  block mb-1">Approved By</span>
            ...
            {adjustment.posted_status === "posted" && (
              <div className="border-t border-gray-100  mt-6 pt-4 flex flex-wrap gap-3">
                <Link
                  to={`/inventory/stock-ledger?product_id=${adjustment.lines?.[0]?.product_id || ""}&warehouse_id=${adjustment.warehouse_id}`}
                  className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-brand-600 bg-brand-50 hover:bg-brand-100 rounded-lg    transition-colors border border-transparent "
                >
                  View Stock Ledger
                </Link>
                <Link
          ...
          <div className="bg-white  rounded-xl shadow-sm border border-gray-100  overflow-hidden">
            <div className="p-4 border-b border-gray-100  bg-gray-50/50 ">
              <h3 className="text-base font-semibold text-gray-900 ">Adjustment Lines</h3>
            </div>
                    ...
                    return (
                      <tr key={line.id} className="hover:bg-gray-50  transition-colors">
                        <td className="px-4 py-3">
                          <p className="font-medium text-gray-900 ">{line.product?.product_name}</p>
                          <p className="text-xs text-gray-500">{line.product?.product_code}</p>
                        </td>
                        <td className="px-4 py-3">
                          {line.product_batch || line.batch ? (
                            <>
                              <p className="font-medium text-gray-900 ">{(line.product_batch || line.batch).batch_number}</p>
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className="text-sm text-gray-700 ">{formatCurrency(line.unit_cost || 0)}</span>
                        </td>
        ...
        <div className="space-y-6">
          <div className="bg-white  rounded-xl shadow-sm border border-gray-100  p-5">
            <h3 className="text-base font-semibold text-gray-900  mb-4">Approval History</h3>
            
            {adjustment.approvals && adjustment.approvals.length > 0 ? (
              <div className="space-y-4">
                {adjustment.approvals.map((approval: any) => (
                  <div key={approval.id} className="relative pl-4 pb-4 border-l border-gray-200  last:border-0 last:pb-0">
