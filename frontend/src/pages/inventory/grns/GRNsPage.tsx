...
const GRNsPage = () => {
  ...
  const columns = [
    { header: "GRN Number", accessorKey: "grn_number" },
    {
      ...
      cell: ({ row }: { row?: any }) => formatDate(row.original.grn_date),
    },
    {
      header: "Supplier",
    ...
    },
    {
      ...
      cell: ({ row }: { row?: any }) =>
        ...
        `Warehouse #${row.original.warehouse_id}`,
    },
    { header: "Supp. Inv", accessorKey: "supplier_invoice_number" },
    {
      ...
      cell: ({ row }: { row?: any }) =>
        formatNumber(row.original.total_stock_quantity, 3),
    },
    {
      header: "Total Amount",
    ...
    },
    {
      ...
      cell: ({ row }: { row?: any }) => (
        <GRNStatusBadge status={row.original.approval_status} />
      ),
    },
    {
      header: "Posted",
      ...
      cell: ({ row }: { row?: any }) => (
        <GRNPostedStatusBadge status={row.original.posted_status} />
      ),
    },
  ];
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage supplier deliveries and GRNs
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-navy-700 dark:bg-navy-800">
        <DataTable
          ...
          onEdit={
            hasPermission("inventory.grn.update") ? handleEdit : undefined
          }
          onDelete={
            hasPermission("inventory.grn.delete") ? handleDelete : undefined
          }
