...
const GRNsPage = () => {
  ...
  const columns = [
    ...
    { header: "Date", accessorKey: "grn_date", cell: ({ row }) => formatDate(row.original.grn_date) },
    { header: "Supplier", accessorKey: "supplier_id", cell: ({ row }) => row.original.supplier?.supplier_name || `Supplier #${row.original.supplier_id}` },
    { header: "Warehouse", accessorKey: "warehouse_id", cell: ({ row }) => row.original.warehouse?.warehouse_name || `Warehouse #${row.original.warehouse_id}` },
    { header: "Supp. Inv", accessorKey: "supplier_invoice_number" },
    { header: "Total Stock", accessorKey: "total_stock_quantity", cell: ({ row }) => formatNumber(row.original.total_stock_quantity, 3) },
    { header: "Total Amount", accessorKey: "total_amount", cell: ({ row }) => formatCurrency(row.original.total_amount) },
    { header: "Approval", accessorKey: "approval_status", cell: ({ row }) => <GRNStatusBadge status={row.original.approval_status} /> },
    { header: "Posted", accessorKey: "posted_status", cell: ({ row }) => <GRNPostedStatusBadge status={row.original.posted_status} /> },
  ];
