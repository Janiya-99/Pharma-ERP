        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {hasPermission("inventory.stock_ledger.view") && grn.posted_status === "posted" && (
            <Link
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50     shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
            <h3 className="text-lg font-semibold text-gray-900  mb-4 border-b border-gray-100  pb-2">Document Info</h3>
            
            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6 text-sm">
              <div>
                <span className="block text-gray-500  mb-1">Branch</span>
                <span className="font-medium text-gray-900 ">{grn.branch?.branch_name || `#${grn.branch_id}`}</span>
              </div>
              <div>
                <span className="block text-gray-500  mb-1">Warehouse</span>
                <span className="font-medium text-gray-900 ">{grn.warehouse?.warehouse_name || `#${grn.warehouse_id}`}</span>
              </div>
              <div>
                <span className="block text-gray-500  mb-1">Supplier Inv. No</span>
                <span className="font-medium text-gray-900 ">{grn.supplier_invoice_number || "N/A"}</span>
              </div>
              <div>
                <span className="block text-gray-500  mb-1">Supplier Inv. Date</span>
                <span className="font-medium text-gray-900 ">{grn.supplier_invoice_date ? formatDate(grn.supplier_invoice_date) : "N/A"}</span>
              </div>
              <div>
                <span className="block text-gray-500  mb-1">PO Number</span>
                <span className="font-medium text-gray-900 ">{grn.purchase_order_number || "N/A"}</span>
              </div>
              <div>
                <span className="block text-gray-500  mb-1">Ref Number</span>
                <span className="font-medium text-gray-900 ">{grn.reference_number || "N/A"}</span>
              </div>
              <div className="col-span-2 md:col-span-3">
                <span className="block text-gray-500  mb-1">Remarks</span>
                <span className="font-medium text-gray-900  whitespace-pre-wrap">{grn.remarks || "None"}</span>
              </div>
            </div>
          </div>
          ...
          {grn.approvals && grn.approvals.length > 0 && (
            <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
              <h3 className="text-lg font-semibold text-gray-900  mb-4 border-b border-gray-100  pb-2">Approval History</h3>
              <div className="space-y-4">
                {grn.approvals.map((appr: unknown) => (
                  <div key={appr.id} className="flex gap-4 p-3 rounded-lg bg-gray-50 ">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-sm text-gray-900  capitalize">{appr.action}</span>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-gray-50  rounded-xl p-4 border border-gray-100 ">
            <h3 className="font-semibold text-gray-800  mb-3 text-sm border-b pb-2 ">Financial Summary</h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center">
                <span className="text-gray-500 ">Total Quantity:</span>
                <span className="font-medium text-gray-800 ">{formatNumber(grn.total_quantity, 3)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500 ">Total Free Qty:</span>
                <span className="font-medium text-gray-800 ">{formatNumber(grn.total_free_quantity, 3)}</span>
              </div>
              <div className="flex justify-between items-center font-bold">
                <span className="text-brand-600 ">Total Stock Qty:</span>
                <span className="text-brand-600 ">{formatNumber(grn.total_stock_quantity, 3)}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-gray-800 ">{formatCurrency(grn.subtotal_amount)}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-gray-800 ">{formatCurrency(grn.tax_amount)}</span>
              </div>
              
              <div className="flex justify-between items-center font-bold pt-2 border-t border-gray-200  text-base">
                <span className="text-brand-600 ">Total Amount:</span>
                <span className="text-brand-600 ">{formatCurrency(grn.total_amount)}</span>
              </div>
            </div>
          </div>

          <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
            <h3 className="text-sm font-semibold text-gray-900  mb-4 border-b border-gray-100  pb-2">Record Timestamps</h3>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Created:</span>
                <span className="text-gray-900 ">{formatDateTime(grn.created_at)}</span>
              </div>
              {grn.approved_at && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Approved:</span>
                  <span className="text-gray-900 ">{formatDateTime(grn.approved_at)}</span>
                </div>
              )}
              {grn.posted_at && (
