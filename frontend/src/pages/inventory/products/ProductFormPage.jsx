...
const ProductFormPage = () => {
  ...
  const updateBarcode = (index, field, value) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      barcodes: formData.barcodes.filter(b => b.barcode.trim() !== "")
    };
        ...
        <div className="bg-white dark:bg-navy-800 rounded-2xl shadow-sm border border-gray-100 dark:border-navy-700 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <ProductCategorySelect value={formData.category_id} onChange={(v) => handleCustomChange("category_id", v)} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Base Unit <span className="text-red-500">*</span></label>
              <ProductUnitSelect value={formData.base_unit_id} onChange={(v) => handleCustomChange("base_unit_id", v)} />
            </div>
            <div>
        ...
        <div className="bg-white dark:bg-navy-800 rounded-2xl shadow-sm border border-gray-100 dark:border-navy-700 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              ...
              <GenericNameSelect value={formData.generic_name_id} onChange={(v) => handleCustomChange("generic_name_id", v)} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Dosage Form</label>
              <DosageFormSelect value={formData.dosage_form_id} onChange={(v) => handleCustomChange("dosage_form_id", v)} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Manufacturer</label>
              <ManufacturerSelect value={formData.manufacturer_id} onChange={(v) => handleCustomChange("manufacturer_id", v)} />
            </div>
            <div>
            ...
            <div className="space-y-3">
              {formData.barcodes.map((bc, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  <input type="text" placeholder="Barcode" value={bc.barcode} onChange={(e) => updateBarcode(idx, "barcode", e.target.value)} className="flex-1 px-3 py-2 border rounded-xl bg-white dark:bg-navy-700 text-sm" />
                  <select value={bc.barcode_type} onChange={(e) => updateBarcode(idx, "barcode_type", e.target.value)} className="w-32 px-3 py-2 border rounded-xl bg-white dark:bg-navy-700 text-sm">
