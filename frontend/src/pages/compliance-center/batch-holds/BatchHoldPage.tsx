        ...
        <div className="flex flex-1 items-center gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800 pl-10 pr-4 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all"
            />
          </div>
        </div>
                ...
                <div className="space-y-2 flex-1">
                  <div className="flex flex-wrap items-center gap-2.5">
                    ...
                    {item.severity === "CRITICAL" && (
                      <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300 font-bold px-2.5 py-0.5 text-[10px]">
                        <ShieldAlert className="h-3 w-3" /> CRITICAL SEVERITY
                      </span>
                    )}
                    {item.holdReason === "TEMP_EXCURSION" && (
                      <span className="rounded bg-sky-100 dark:bg-sky-950/60 text-sky-800 dark:text-sky-300 font-bold px-2 py-0.5 text-[10px]">
                        ❄️ Cold Chain Excursion
                      </span>
                    )}
                    {item.holdReason === "QC_ASSAY_FAIL" && (
                      <span className="rounded bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 font-bold px-2 py-0.5 text-[10px]">
                        🧪 QC Assay Variation
                      </span>
                    )}
                    {item.holdReason === "PACKAGING_DEFECT" && (
                      <span className="rounded bg-purple-100 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300 font-bold px-2 py-0.5 text-[10px]">
                  ...
                  {/* Notes / Evidence */}
                  <div className="mt-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 p-3.5 text-xs text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-800">
                    <span className="font-bold text-slate-900 dark:text-white">Investigation Status & Notes: </span>
                    {item.notes}
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end justify-between gap-4 border-t lg:border-t-0 pt-4 lg:pt-0 border-slate-100 dark:border-slate-800">
                  <div>
                    {item.status === "QUARANTINED" && (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-400 border border-amber-200 dark:border-amber-800 px-3 py-1 text-xs font-bold">
                        <Clock className="h-3.5 w-3.5" /> QUARANTINED (LOCKED)
                      </span>
                    )}
                    {item.status === "INVESTIGATION_IN_PROGRESS" && (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 px-3 py-1 text-xs font-bold animate-pulse">
                    ...
                    {item.status === "MARKED_FOR_DISPOSAL" && (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-50 text-rose-700 dark:bg-rose-950/50 dark:text-rose-400 border border-rose-200 dark:border-rose-800 px-3 py-1 text-xs font-bold">
                  ...
                  {(item.status === "QUARANTINED" || item.status === "INVESTIGATION_IN_PROGRESS") && (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-950/50 px-3.5 py-2 text-xs font-semibold text-rose-700 dark:text-rose-300 hover:bg-rose-100 transition-colors"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-in fade-in-50">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-2xl space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-2.5">
                <div className={`rounded-lg p-2 ${modalAction === "RELEASE" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50" : "bg-rose-50 text-rose-600 dark:bg-rose-950/50"}`}>
            ...
            <div className="space-y-3 text-xs bg-slate-50 dark:bg-slate-800/50 p-3.5 rounded-xl border border-slate-100 dark:border-slate-800">
            ...
            <form onSubmit={handleConfirmAction} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 p-3.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
