    ...
    <div className="w-full space-y-8 animate-in fade-in-50 duration-300">
      ...
      <div className="flex flex-col gap-4 border-b border-slate-200/80 dark:border-slate-800 pb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <button className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 transition-all shadow-xs">
      ...
      <div className="rounded-2xl border border-indigo-200/80 dark:border-indigo-900/60 bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-transparent p-6 relative overflow-hidden shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 dark:bg-emerald-950/60 px-2.5 py-0.5 text-[11px] font-bold text-emerald-800 dark:text-emerald-300 border border-emerald-300/50 dark:border-emerald-800/50">
          ...
          <div className="text-right sm:border-l sm:border-indigo-200 dark:sm:border-indigo-800 sm:pl-6">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Verified Records</span>
            <p className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{data.length}</p>
                ...
                filteredData.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 py-4">
                      <p className="font-mono font-bold text-xs text-indigo-600 dark:text-indigo-400">{row.recordNumber}</p>
                      <span className="text-[11px] text-slate-400 font-medium">{row.timestamp}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="inline-flex items-center gap-1 rounded-md bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-xs font-bold text-slate-700 dark:text-slate-300">
                    ...
                    <td className="px-5 py-4 max-w-md">
                      <p className="font-bold text-slate-800 dark:text-slate-200 text-xs">{row.subject}</p>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-0.5">
                        Authority: <b>{row.authority}</b> • By: {row.performedBy}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 font-mono text-[11px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/80 px-2.5 py-1 rounded-md border border-slate-200/60 dark:border-slate-700/60 max-w-xs truncate">
                    ...
                    <td className="px-5 py-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-indigo-50 hover:text-indigo-600 dark:hover:bg-indigo-950/50 transition-colors"
                      >
      ...
      {selectedRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-in fade-in-50">
          <div className="w-full max-w-2xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-2xl space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Regulatory Inspection Dossier</h3>
            ...
            <div className="space-y-4 text-xs">
              <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4 border border-slate-100 dark:border-slate-800 space-y-2">
                <h4 className="font-bold text-slate-900 dark:text-white uppercase tracking-wider text-[11px] text-indigo-600 dark:text-indigo-400">
                  Record Overview
                </h4>
                <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{selectedRecord.subject}</p>
              ...
              {/* Cryptographic Proof */}
              <div className="rounded-xl bg-emerald-50/50 dark:bg-emerald-950/20 p-4 border border-emerald-200/60 dark:border-emerald-800/60 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-emerald-100 dark:bg-emerald-900 px-2 py-0.5 font-bold text-[10px] text-emerald-800 dark:text-emerald-200">
                    VERIFIED IMMUTABLE
                  </span>
                </div>
                <p className="font-mono text-[11px] text-slate-700 dark:text-slate-300 break-all bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-emerald-200/40 dark:border-emerald-800/40">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50"
              >
