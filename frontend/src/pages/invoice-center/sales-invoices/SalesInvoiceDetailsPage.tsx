...
const SalesInvoiceDetailsPage: React.FC = () => {
  ...
  const fetchInvoice = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Failed to fetch sales invoice.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!invoice) {
    return <div className="p-8 text-center text-gray-500">Sales Invoice not found.</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" onClick={() => navigate("/invoice-center/sales-invoices")}>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="font-medium">{invoice.due_date ? new Date(invoice.due_date).toLocaleDateString() : "-"}</div>
                </div>
                <div>
                  <div className="text-gray-500">Branch</div>
                  <div className="font-medium">{invoice.branch?.branch_name || "-"}</div>
                </div>
                <div>
                  <div className="text-gray-500">Warehouse</div>
                  <div className="font-medium">{invoice.warehouse?.warehouse_name || "-"}</div>
                </div>
                <div>
                  <div className="text-gray-500">Customer Reference</div>
                  <div className="font-medium">{invoice.customer_reference_number || "-"}</div>
                </div>
                <div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                {invoice.posted_status === "posted" && (
                  <Button variant="outline" className="w-full text-xs" onClick={() => window.open("/inventory/stock-ledger?source_type=sales_invoice&source_number=" + invoice.invoice_number)}>
                    View Stock Ledger
                  </Button>
                )}
              </div>
      ...
      {/* Workflow Modals */}
      <SubmitSalesInvoiceModal isOpen={isSubmitOpen} onClose={() => setIsSubmitOpen(false)} invoiceId={invoice.id} onSuccess={fetchInvoice} />
      <ApproveSalesInvoiceModal isOpen={isApproveOpen} onClose={() => setIsApproveOpen(false)} invoiceId={invoice.id} onSuccess={fetchInvoice} showCreditWarning={(invoice.customer?.credit_limit || 0) > 0 && ((invoice.customer?.current_balance || 0) + invoice.total_amount) > (invoice.customer?.credit_limit || 0)} />
      <RejectSalesInvoiceModal isOpen={isRejectOpen} onClose={() => setIsRejectOpen(false)} invoiceId={invoice.id} onSuccess={fetchInvoice} />
      <PostSalesInvoiceConfirmModal 
        isOpen={isPostOpen} 
        ...
        totalQuantity={invoice.lines?.reduce((sum, line) => sum + Number(line.quantity), 0) || 0}
        customerName={invoice.customer?.customer_name || `Customer #${invoice.customer_id}`}
        warehouseName={invoice.warehouse?.warehouse_name || `Warehouse #${invoice.warehouse_id}`}
        onSuccess={fetchInvoice} 
      />
      <CancelSalesInvoiceModal isOpen={isCancelOpen} onClose={() => setIsCancelOpen(false)} invoiceId={invoice.id} onSuccess={fetchInvoice} />
      <DeleteSalesInvoiceConfirmModal 
        isOpen={isDeleteOpen} 
