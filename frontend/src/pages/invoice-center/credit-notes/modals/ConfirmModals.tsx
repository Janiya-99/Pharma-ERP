...
export const PostCreditNoteConfirmModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onConfirm,
}) => {
      ...
      <DialogContent>
        ...
        <Alert className="my-4 border-amber-200 bg-amber-50 text-amber-900">
          ...
          <AlertDescription className="mt-2 text-sm text-amber-700">
            Posting this credit note will permanently update the customer's
...
export const DeleteCreditNoteConfirmModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onConfirm,
}) => {
    ...
    <Dialog open={isOpen} onOpenChange={(open: boolean) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete this credit note? This action cannot
            be undone.
          </DialogDescription>
        </DialogHeader>
