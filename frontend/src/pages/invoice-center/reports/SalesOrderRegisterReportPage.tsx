...
export const SalesOrderRegisterReportPage: React.FC = () => {
  ...
  const [data, setData] = useState<{
    summary: any;
    rows: SalesOrderRegisterReportRow[];
  } | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await invoiceCenterApi.getSalesOrderRegisterReport(
        cleanFilters
      );
  ...
  const columns: ColumnDef<SalesOrderRegisterReportRow>[] = [
    ...
    },
    {
      ...
      cell: (row) => (
        <Badge
          variant={row.order_status === "closed" ? "secondary" : "default"}
        >
          {row.order_status}
        </Badge>
      ),
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(val) =>
                setFilters((prev) => ({ ...prev, order_status: val }))
              }
            >
