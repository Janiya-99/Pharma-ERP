...
import { CreditNoteRegisterReportParams, CreditNoteRegisterReportRow } from "@/types/invoice-center-reports";
import { toast } from "sonner";
import { FileDown, Coins, UploadCloud, DownloadCloud } from "lucide-react";

export const CreditNoteRegisterReportPage: React.FC = () => {
  const [filters, setFilters] = useState<CreditNoteRegisterReportParams>({
    page: 1,
    limit: 10,
    search: "",
    credit_note_type: "all",
    approval_status: "all",
    posted_status: "all",
    finance_post_status: "all",
  });
  
  const [data, setData] = useState<{ summary: any, rows: CreditNoteRegisterReportRow[] } | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      if (cleanFilters.credit_note_type === "all") delete cleanFilters.credit_note_type;
      ...
      const res = await invoiceCenterApi.getCreditNoteRegisterReport(cleanFilters);
  ...
  const handleResetFilters = () => {
    setFilters({ 
      ...
      credit_note_type: "all", approval_status: "all", 
      posted_status: "all", finance_post_status: "all" 
    });
  ...
  const columns: ColumnDef<CreditNoteRegisterReportRow>[] = [
    { header: "CN Number", accessorKey: "credit_note_number" },
    { header: "Date", accessorKey: "credit_note_date" },
    { header: "Customer Name", accessorKey: "customer_name" },
    { header: "Inv No.", accessorKey: "sales_invoice_number" },
    { 
      header: "Type", 
      cell: (row) => (
        <Badge variant="outline">{row.credit_note_type}</Badge>
      ),
      align: "center"
    },
    { 
  ...
  const summaryCards: SummaryCardItem[] = [
    { title: "Credit Note Count", value: summary.credit_note_count || 0, format: "number", icon: <FileDown className="h-4 w-4" /> },
    { title: "Total Amount", value: summary.total_amount || 0, format: "currency", icon: <Coins className="h-4 w-4 text-emerald-500" /> },
    { title: "Finance Posted", value: summary.finance_posted_count || 0, format: "number", icon: <UploadCloud className="h-4 w-4 text-blue-500" /> },
    { title: "Finance Unposted", value: summary.finance_unposted_count || 0, format: "number", icon: <DownloadCloud className="h-4 w-4 text-amber-500" /> },
  ];

  return (
    <ReportPageShell
      title="Credit Note Register"
      description="View all credit notes and their statuses."
      actions={
        <ReportExportActions 
          data={data} 
          filename={`credit-note-register-${new Date().toISOString().split('T')[0]}`} 
      ...
      filters={
        ...
        >
          <div className="space-y-1">
            ...
            <Input 
              id="search" 
              placeholder="Search credit notes, customers..."
          ...
          <div className="space-y-1">
            ...
            <Select 
              value={filters.credit_note_type || "all"} 
              onValueChange={(val) => setFilters(prev => ({ ...prev, credit_note_type: val }))}
            >
              <SelectTrigger id="credit_note_type">
