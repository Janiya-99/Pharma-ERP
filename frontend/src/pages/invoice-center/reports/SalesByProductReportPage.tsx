...
import { SalesByProductReportParams, SalesByProductReportRow } from "@/types/invoice-center-reports";
...
export const SalesByProductReportPage: React.FC = () => {
  ...
  const [data, setData] = useState<{ summary: any, rows: SalesByProductReportRow[] } | null>(null);
  ...
  const columns: ColumnDef<SalesByProductReportRow>[] = [
    ...
    { 
      ...
      cell: (row) => <span className="font-medium">{formatNumber(row.quantity_sold)}</span>,
      align: "right"
    },
    { 
      header: "Avg Price", 
  ...
  const summaryCards: SummaryCardItem[] = [
    { title: "Products Sold", value: summary.product_count || 0, format: "number", icon: <Package className="h-4 w-4" /> },
    { title: "Total Quantity", value: summary.total_quantity || 0, format: "number", icon: <Hash className="h-4 w-4" /> },
    { title: "Total Revenue", value: summary.total_revenue || 0, format: "currency", icon: <Coins className="h-4 w-4 text-emerald-500" /> },
    { title: "Avg Order Value", value: summary.total_revenue && summary.product_count ? (summary.total_revenue / summary.product_count) : 0, format: "currency", icon: <Percent className="h-4 w-4 text-blue-500" /> },
  ];
