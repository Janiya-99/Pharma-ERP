...
export const InvoiceCenterReportsDashboardPage: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const res = await invoiceCenterApi.getInvoiceCenterDashboardSummary(
        filters
      );
  ...
  const customerCards: SummaryCardItem[] = [
    ...
    },
    {
      ...
      format: "number",
      icon: <Building2 className="text-emerald-500 h-4 w-4" />,
    },
    {
      title: "Blocked Customers",
      value: data?.blocked_customers || 0,
      format: "number",
      icon: <UserX className="h-4 w-4 text-red-500" />,
    },
    {
      title: "Over Credit Limit",
      value: data?.customers_over_credit_limit || 0,
      format: "number",
  ...
  const salesOrderCards: SummaryCardItem[] = [
    {
      ...
      value: data?.total_sales_orders || 0,
      format: "number",
      icon: <ShoppingCart className="h-4 w-4" />,
    },
    {
      title: "Approved Orders",
      value: data?.approved_sales_orders || 0,
      format: "number",
    ...
    {
      title: "Closed Orders",
      value: data?.closed_sales_orders || 0,
      format: "number",
      icon: <XCircle className="text-slate-500 h-4 w-4" />,
    },
  ];
  ...
  const invoiceCards: SummaryCardItem[] = [
    {
      ...
      value: data?.total_sales_invoices || 0,
      format: "number",
      icon: <FileText className="h-4 w-4" />,
    },
    {
      title: "Posted Invoices",
      value: data?.posted_sales_invoices || 0,
      format: "number",
  ...
  const collectionCards: SummaryCardItem[] = [
    ...
    },
    {
      ...
      value: data?.total_balance_amount || 0,
      format: "currency",
      icon: <Banknote className="h-4 w-4 text-amber-500" />,
    },
    {
      title: "Allocated Receipts",
      value: data?.total_allocated_receipt_amount || 0,
      format: "currency",
  ...
  const financeCards: SummaryCardItem[] = [
    {
      ...
      value: data?.finance_posted_documents || 0,
      format: "number",
      icon: <UploadCloud className="text-emerald-500 h-4 w-4" />,
    },
    {
      title: "Finance Unposted",
      value: data?.finance_unposted_documents || 0,
      format: "number",
  ...
  return (
    <ReportPageShell
      ...
      actions={
        <ReportExportActions
          data={data}
          filename={`invoice-center-dashboard-${
      ...
      summary={
        <div className="space-y-6">
          <div>
            <h3 className="mb-3 flex items-center text-lg font-medium text-navy-900">
          ...
          <div>
            <h3 className="mb-3 flex items-center text-lg font-medium text-navy-900">
              <ShoppingCart className="mr-2 h-5 w-5" /> Sales Orders
            </h3>
          ...
          <div>
            <h3 className="mb-3 flex items-center text-lg font-medium text-navy-900">
              <Wallet className="mr-2 h-5 w-5" /> Collections
            </h3>
            <ReportSummaryCards items={collectionCards} isLoading={isLoading} />
          </div>
          <div>
            <h3 className="mb-3 flex items-center text-lg font-medium text-navy-900">
              <Activity className="mr-2 h-5 w-5" /> Finance Posting
            </h3>
