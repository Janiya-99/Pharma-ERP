...
import { FinancePostingStatusReportParams, FinancePostingStatusReportRow } from "@/types/invoice-center-reports";
import { toast } from "sonner";
import { UploadCloud, DownloadCloud, AlertTriangle, FileCheck } from "lucide-react";
...
export const FinancePostingStatusReportPage: React.FC = () => {
  ...
  const [data, setData] = useState<{ summary: any, rows: FinancePostingStatusReportRow[] } | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      if (cleanFilters.document_type === "all") delete cleanFilters.document_type;
      if (cleanFilters.finance_post_status === "all") delete cleanFilters.finance_post_status;

      const res = await invoiceCenterApi.getFinancePostingStatusReport(cleanFilters);
  ...
  const handleResetFilters = () => {
    setFilters({ 
      ...
      document_type: "all", finance_post_status: "all"
    });
  ...
  const columns: ColumnDef<FinancePostingStatusReportRow>[] = [
    ...
    { 
      header: "Amount", 
      cell: (row) => <span className="font-semibold text-navy-900">{formatMoney(row.document_amount)}</span>,
      align: "right"
    },
    { 
      header: "Posting Status", 
      cell: (row) => (
        <Badge variant={row.finance_post_status === 'posted' ? 'default' : row.finance_post_status === 'failed' ? 'destructive' : 'secondary'}>
  ...
  const summaryCards: SummaryCardItem[] = [
    { title: "Total Documents", value: summary.total_documents || 0, format: "number", icon: <FileCheck className="h-4 w-4" /> },
    { title: "Posted to Finance", value: summary.posted_count || 0, format: "number", icon: <UploadCloud className="h-4 w-4 text-emerald-500" /> },
    { title: "Unposted Documents", value: summary.unposted_count || 0, format: "number", icon: <DownloadCloud className="h-4 w-4 text-amber-500" /> },
    { title: "Failed Postings", value: summary.failed_count || 0, format: "number", icon: <AlertTriangle className="h-4 w-4 text-red-500" /> },
  ];
  ...
  return (
    <ReportPageShell
      ...
      actions={
        <ReportExportActions 
          data={data} 
          filename={`finance-posting-status-${new Date().toISOString().split('T')[0]}`} 
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <Select 
              ...
              onValueChange={(val) => setFilters(prev => ({ ...prev, document_type: val }))}
            >
          ...
          <div className="space-y-1">
            ...
            <Select 
              ...
              onValueChange={(val) => setFilters(prev => ({ ...prev, finance_post_status: val }))}
            >
