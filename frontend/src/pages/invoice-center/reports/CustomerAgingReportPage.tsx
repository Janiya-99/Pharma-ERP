...
import { CustomerAgingReportParams, CustomerAgingReportRow } from "@/types/invoice-center-reports";
...
export const CustomerAgingReportPage: React.FC = () => {
  ...
  const handleResetFilters = () => {
    setFilters({ page: 1, limit: 10, search: "", as_of_date: new Date().toISOString().split('T')[0] });
  ...
  const columns: ColumnDef<CustomerAgingReportRow>[] = [
    ...
    { 
      header: "> 120 Days", 
      cell: (row) => (
        <span className={row.over_120 > 0 ? "text-red-600 font-semibold" : ""}>
          {formatMoney(row.over_120)}
          {row.over_120 > 0 && <Badge variant="destructive" className="ml-2 py-0">!</Badge>}
        </span>
      ),
      align: "right"
    },
