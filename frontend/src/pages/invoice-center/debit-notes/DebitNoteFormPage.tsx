...
import {
  DebitNoteLinesTable,
  createBlankDebitNoteLine,
} from "./DebitNoteLinesTable";
import {
  DebitNoteTotalsCard,
...
const DebitNoteFormPage: React.FC = () => {
  ...
  const loadDebitNote = async () => {
    ...
    try {
      ...
      setValue(
        "debit_note_date",
        data.debit_note_date ? data.debit_note_date.slice(0, 10) : ""
      );
  ...
  const onSubmit = async (data: FormValues) => {
    if (
      ...
      toast.error(
        `Cannot create debit note for a customer with status: ${customer.status}`
      );
      return;
    }
    ...
    <div className="mx-auto max-w-6xl space-y-6 pb-20">
      <div className="flex items-center justify-between">
        ...
        <PermissionGuard
          permission={
            isEdit
              ? "invoice_center.debit_note.update"
              : "invoice_center.debit_note.create"
          }
        >
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label
                      htmlFor="debit_note_date"
                      className={errors.debit_note_date ? "text-red-500" : ""}
                    >
                      Date *
                    </Label>
                    <DatePicker
                    ...
                    {errors.debit_note_date && (
                      <span className="text-xs text-red-500">
                        {errors.debit_note_date.message}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <Label
                      ...
                      className={errors.debit_note_type ? "text-red-500" : ""}
                    >
                      Type *
                    </Label>
                    <Select
                      onValueChange={(val) =>
                        setValue("debit_note_type", val as any)
                      }
                      defaultValue={watch("debit_note_type")}
                    >
                      <SelectTrigger
                        className={
                          errors.debit_note_type ? "border-red-500" : ""
                        }
                      >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="freight_charge">
                          Freight Charge
                        </SelectItem>
                        <SelectItem value="tax_adjustment">
                          Tax Adjustment
                        </SelectItem>
                        <SelectItem value="other">Other</SelectItem>
                      </SelectContent>
                    </Select>
                    {errors.debit_note_type && (
                      <span className="text-xs text-red-500">
                        {errors.debit_note_type.message}
                      </span>
                    )}
                  </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    If this debit note is linked to a sales invoice, enter the
          ...
          <div className="space-y-6">
            ...
            {customer && (
              <DebitNoteCreditLimitWarningCard
                projectedBalance={
                  Number(customer.current_balance || 0) + totals.total
                }
            ...
            <DebitNoteBalanceImpactCard
              ...
              invoiceBalanceAfter={
                salesInvoiceInfo
                  ? Number(salesInvoiceInfo.balance_amount) + totals.total
                  : null
              }
            />
