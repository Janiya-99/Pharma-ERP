...
const PostToFinanceConfirmModal: React.FC<Props> = ({
  open,
  onOpenChange,
  document,
  onSuccess,
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setResult(data);
        toast.success(
          res.data.message || "Document posted to Finance successfully."
        );
        onSuccess();
      } else {
      ...
      const msg =
        error.response?.data?.message || "Failed to post document to Finance";
      toast.error(msg);
    } finally {
      setPosting(false);
    }
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                ...
                <div>
                  ...
                  <Badge
                    variant="outline"
                    className="mt-0.5 border-transparent bg-gray-200 text-gray-800"
                  >
              ...
              {result ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <p className="font-mono text-sm font-semibold text-green-800">
                        {result.finance_reference_number}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-green-800">
                        {formatCurrency(result.debit_total)}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-green-600">Credit Total</p>
                      <p className="text-sm font-semibold text-green-800">
                        {formatCurrency(result.credit_total)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  <p className="text-xs text-amber-800">
                    ...
                    Posting this document to Finance will create General Ledger
                    entries and mark the document as finance posted. This action
                    cannot be repeated.
                  </p>
                </div>
              )}
