...
const PendingFinancePostingsPage: React.FC<Props> = ({ onPostingComplete }) => {
  ...
  const [selectedDocument, setSelectedDocument] =
    useState<PendingFinancePosting | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to load pending postings"
      );
    } finally {
      setLoading(false);
    }
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="font-medium text-gray-500">
            No pending finance postings found
          </p>
          <p className="mt-1 text-xs text-gray-400">
            All operationally posted documents have been posted to Finance, or
          ...
          <div className="overflow-hidden rounded-lg border">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {data.map((item, idx) => (
                  <tr
                    key={`${item.document_type}-${item.document_id}-${idx}`}
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <span className="text-xs text-gray-500">
                        {item.operational_posted_status}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <FinancePostStatusBadge
                        status={item.finance_post_status}
                      />
                    </td>
