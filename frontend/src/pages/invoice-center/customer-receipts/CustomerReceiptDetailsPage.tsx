...
const CustomerReceiptDetailsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to load receipt details"
      );
    } finally {
      setLoading(false);
    }
  ...
  if (!data) {
    return (
      <div className="p-8 text-center text-red-500">Receipt not found.</div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate("/admin/invoice-center/customer-receipts")}
          >
        ...
        {!isCancelled && (
          <CustomerReceiptActionButtons
            ...
            onEdit={() =>
              navigate(
                `/admin/invoice-center/customer-receipts/${data.id}/edit`
              )
            }
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              {data.remarks && (
                <div className="col-span-2">
                  ...
                  <p className="rounded-md border border-gray-100 bg-gray-50 p-3 text-gray-900">
                    {data.remarks}
                  </p>
                </div>
              )}
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-gray-50 font-medium text-gray-500">
                    <tr>
                      ...
                      <th className="bg-green-50 px-6 py-3 text-right text-green-700">
                        Allocated Amount
                      </th>
                    ...
                    {allocations.map((alloc: any) => (
                      <tr key={alloc.id} className="hover:bg-gray-50">
                        <td className="px-6 py-3 font-medium text-brand-600">
                          {alloc.sales_invoice?.invoice_number ||
                            alloc.sales_invoice_id}
                        </td>
                        <td className="px-6 py-3 text-right">
                          {formatCurrency(
                            alloc.sales_invoice?.total_amount || 0
                          )}
                        </td>
                        <td className="px-6 py-3 text-right text-gray-500">
                          {formatCurrency(
                            alloc.sales_invoice?.paid_amount || 0
                          )}
                        </td>
                        <td className="bg-green-50/30 px-6 py-3 text-right font-medium text-green-700">
                          {formatCurrency(alloc.allocated_amount)}
                        </td>
                        <td className="px-6 py-3 text-right font-medium text-gray-900">
                          {formatCurrency(
                            (alloc.sales_invoice?.balance_amount || 0) -
        ...
        <div className="space-y-6">
          ...
          {customer && (
            <CustomerReceiptCustomerBalanceCard
              ...
              unallocatedAmount={data.unallocated_amount}
              balanceAfterReceipt={
                (customer.current_balance || 0) - data.allocated_amount
              }
          ...
          <div className="space-y-2 rounded-xl border border-gray-100 bg-gray-50 p-4 text-xs text-gray-500">
