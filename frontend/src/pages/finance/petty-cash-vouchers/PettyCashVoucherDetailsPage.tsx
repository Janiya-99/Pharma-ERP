...
export default function PettyCashVoucherDetailsPage() {
  ...
  const fetchVoucher = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to load petty cash voucher details");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAction = async (actionFn: unknown, payload: Record<string, unknown>, modalName: unknown) => {
  ...
  if (error || !voucher) return <div className="p-8 text-center text-red-500 font-medium">{error || "Voucher not found."}</div>;
    ...
    <div className="flex flex-col gap-4 py-4 h-full max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-2">
        ...
        <PettyCashActionButtons
          ...
          onEdit={() => history.push(`/admin/finance/petty-cash-vouchers/${voucher.id}/edit`)}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white  rounded-xl shadow-sm border border-gray-100  p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Voucher Type</span>
                <div className="mt-1"><PettyCashVoucherTypeBadge type={voucher.voucher_type} /></div>
              </div>
              <div>
                ...
                <div className="mt-1 flex gap-2">
                  ...
                  {voucher.approval_status === "approved" && <PettyCashPostedStatusBadge status={voucher.posted_status} />}
                </div>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Petty Cash Fund</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.petty_cash_fund?.fund_code} - {voucher.petty_cash_fund?.fund_name}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Branch</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.branch?.branch_name}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Financial Year</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.financial_year?.year_name}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Accounting Period</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.accounting_period?.period_name}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Payee Name</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.payee_name || '-'}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Reference Number</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.reference_number || '-'}</span>
              </div>
              <div className="md:col-span-2">
                <span className="block text-sm font-medium text-gray-500 ">Description</span>
                <span className="block mt-1 text-base text-gray-900 ">{voucher.description || '-'}</span>
              </div>
            </div>
          </div>
            ...
            {(!voucher.approvals || voucher.approvals.length === 0) ? (
              ...
              <div className="space-y-4">
                {voucher.approvals.map((approval: unknown) => (
                  <div key={approval.id} className="relative pl-4 border-l-2 border-gray-200 ">
                    ...
                    <div className="flex flex-col">
                      ...
                      {approval.remarks && (
                        <span className="text-sm mt-1 text-gray-600  bg-gray-50  p-2 rounded-md">
                          "{approval.remarks}"
                        </span>
                      )}
                    </div>
      ...
      <SubmitPettyCashModal isOpen={modals.submit} onClose={() => setModals((m: unknown) => ({ ...m, submit: false }))} isSubmitting={actionLoading} onSubmit={(payload: Record<string, unknown>) => handleAction(financeApi.submitPettyCashVoucher, payload, "submit")} />
      <ApprovePettyCashModal isOpen={modals.approve} onClose={() => setModals((m: unknown) => ({ ...m, approve: false }))} isSubmitting={actionLoading} onApprove={(payload: Record<string, unknown>) => handleAction(financeApi.approvePettyCashVoucher, payload, "approve")} />
      <RejectPettyCashModal isOpen={modals.reject} onClose={() => setModals((m: unknown) => ({ ...m, reject: false }))} isSubmitting={actionLoading} onReject={(payload: Record<string, unknown>) => handleAction(financeApi.rejectPettyCashVoucher, payload, "reject")} />
      <PostPettyCashConfirmModal isOpen={modals.post} onClose={() => setModals((m: unknown) => ({ ...m, post: false }))} isSubmitting={actionLoading} onConfirm={() => handleAction(financeApi.postPettyCashVoucher, undefined, "post")} />
    </div>
  );
}
