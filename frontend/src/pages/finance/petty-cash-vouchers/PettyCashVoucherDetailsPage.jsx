...
export default function PettyCashVoucherDetailsPage() {
  ...
  return (
    <div className="flex flex-col gap-4 py-4 h-full max-w-6xl mx-auto">
          ...
          <div className="bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 p-6">
            ...
            {(!voucher.approvals || voucher.approvals.length === 0) ? (
              ...
              <div className="space-y-4">
                {voucher.approvals.map((approval) => (
      ...
      <SubmitPettyCashModal isOpen={modals.submit} onClose={() => setModals(m => ({ ...m, submit: false }))} isSubmitting={actionLoading} onSubmit={(payload) => handleAction(financeApi.submitPettyCashVoucher, payload, "submit")} />
      <ApprovePettyCashModal isOpen={modals.approve} onClose={() => setModals(m => ({ ...m, approve: false }))} isSubmitting={actionLoading} onApprove={(payload) => handleAction(financeApi.approvePettyCashVoucher, payload, "approve")} />
      <RejectPettyCashModal isOpen={modals.reject} onClose={() => setModals(m => ({ ...m, reject: false }))} isSubmitting={actionLoading} onReject={(payload) => handleAction(financeApi.rejectPettyCashVoucher, payload, "reject")} />
      <PostPettyCashConfirmModal isOpen={modals.post} onClose={() => setModals(m => ({ ...m, post: false }))} isSubmitting={actionLoading} onConfirm={() => handleAction(financeApi.postPettyCashVoucher, undefined, "post")} />
    </div>
  );
}
