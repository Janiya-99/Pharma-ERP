  ...
  return (
    <div className="flex h-full flex-col gap-4 py-4">
      <FinancePageHeader
        ...
        onAdd={
          hasPermission("finance.petty_cash_voucher.create")
            ? () => history.push("/admin/finance/petty-cash-vouchers/create")
            : undefined
        }
      ...
      <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-navy-700 dark:bg-navy-800">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-5">
          <select
            value={filters.petty_cash_fund_id}
            onChange={(e: any) =>
              setFilters((prev: unknown) => ({
                ...prev,
                petty_cash_fund_id: e.target.value,
              }))
            }
            className="w-full rounded-md border px-3 py-2 dark:border-navy-600 dark:bg-navy-900 dark:text-white"
          >
          ...
          <input
            type="text"
            placeholder="Search Payee/Ref/Voucher No..."
      ...
      <div className="flex-1 overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-navy-700 dark:bg-navy-800">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700 dark:divide-navy-700 dark:text-gray-300">
              {loading ? (
                <tr>
                  <td
                    colSpan="9"
                ...
                vouchers.map((v: unknown) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {v.petty_cash_fund?.fund_code}
                    </td>
                    <td className="px-4 py-3">
                      <PettyCashVoucherTypeBadge type={v.voucher_type} />
                    </td>
                    <td className="px-4 py-3">{v.payee_name}</td>
                    <td className="px-4 py-3">{v.reference_number}</td>
                    <td className="px-4 py-3 text-right font-medium">
                      <MoneyDisplay amount={v.total_amount} />
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                        <div className="flex items-center gap-2">
                          {hasPermission("finance.petty_cash_voucher.view") && (
                            <button
                              onClick={() =>
                                history.push(
                                  `/admin/finance/petty-cash-vouchers/${v.id}`
                                )
                              }
                        ...
                        <PettyCashActionButtons
                          ...
                          onEdit={() =>
                            history.push(
                              `/admin/finance/petty-cash-vouchers/${v.id}/edit`
                            )
                          }
                          onDelete={() => handleDelete(v.id)}
                          onSubmit={() => {
                            setActionVoucherId(v.id);
                            setModals((m: unknown) => ({ ...m, submit: true }));
                          }}
                          onApprove={() => {
                            setActionVoucherId(v.id);
                            setModals((m: unknown) => ({
                          ...
                          onReject={() => {
                            setActionVoucherId(v.id);
                            setModals((m: unknown) => ({ ...m, reject: true }));
                          }}
                          onPost={() => {
                            setActionVoucherId(v.id);
                            setModals((m: unknown) => ({ ...m, post: true }));
                          }}
                        />
                      </div>
                    </td>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50 px-4 py-3 text-gray-500 dark:border-navy-700 dark:bg-navy-800 dark:text-gray-400">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded border border-gray-300 px-3 py-1 transition-colors hover:bg-gray-100 disabled:opacity-50 dark:border-navy-600 dark:hover:bg-navy-700"
            >
              Previous
            </button>
      ...
      <SubmitPettyCashModal
        ...
        onSubmit={(payload: Record<string, unknown>) =>
          handleAction(
            financeApi.submitPettyCashVoucher,
            actionVoucherId,
            payload,
            "submit"
          )
      ...
      <ApprovePettyCashModal
        ...
        onApprove={(payload: Record<string, unknown>) =>
          handleAction(
            financeApi.approvePettyCashVoucher,
            actionVoucherId,
            payload,
            "approve"
          )
      ...
      <RejectPettyCashModal
        ...
        onReject={(payload: Record<string, unknown>) =>
          handleAction(
            financeApi.rejectPettyCashVoucher,
            actionVoucherId,
            payload,
            "reject"
          )
      ...
      <PostPettyCashConfirmModal
        ...
        onConfirm={() =>
          handleAction(
            financeApi.postPettyCashVoucher,
            actionVoucherId,
            undefined,
            "post"
          )
