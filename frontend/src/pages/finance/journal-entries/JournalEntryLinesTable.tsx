...
function JournalEntryLinesTable({ lines, setLines, disabled }: { lines?: any; setLines?: any; disabled?: any }) {
  const addLine = useCallback(() => {
    setLines([...lines, { account_id: "", line_description: "", debit_amount: 0, credit_amount: 0 }]);
  }, [lines, setLines]);

  const removeLine = useCallback((index: any) => {
    if (lines.length <= 2) {
  ...
  const clearLine = useCallback((index: any) => {
    const newLines = [...lines];
    newLines[index] = { account_id: "", line_description: "", debit_amount: 0, credit_amount: 0 };
    setLines(newLines);
  }, [lines, setLines]);

  const updateLine = useCallback((index: any, field: any, value: any) => {
    const newLines = [...lines];
  ...
  const totalDebit = useMemo(() => {
    return lines.reduce((sum: any, line: any) => sum + (Number(line.debit_amount) || 0), 0);
  }, [lines]);

  const totalCredit = useMemo(() => {
    return lines.reduce((sum: any, line: any) => sum + (Number(line.credit_amount) || 0), 0);
  }, [lines]);
                ...
                <tr key={index} className="hover:bg-gray-50 dark:hover:bg-navy-700/30">
                  <td className="px-4 py-2">
                    <JournalLineAccountSelect
                      ...
                      onChange={(val: any) => updateLine(index, "account_id", val)}
                      disabled={disabled}
                    />
                  </td>
                  <td className="px-4 py-2">
                  ...
                  <td className="px-4 py-2">
                    <input
                      ...
                      onChange={(e: any) => updateLine(index, "debit_amount", parseFloat(e.target.value) || 0)}
                  ...
                  <td className="px-4 py-2">
                    <input
                      ...
                      onChange={(e: any) => updateLine(index, "credit_amount", parseFloat(e.target.value) || 0)}
