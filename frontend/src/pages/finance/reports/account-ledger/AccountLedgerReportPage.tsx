...
const AccountLedgerReportPage = () => {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to fetch account ledger report"
      );
      setData(null);
    } finally {
      setLoading(false);
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          You do not have permission to view the Account Ledger report.
        </p>
      </div>
    );
  }
      ...
      {!data && !loading && (
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center shadow dark:border-navy-700 dark:bg-navy-800">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Select an account and click Apply Filters to view the account
            ledger.
          </p>
        </div>
      )}
