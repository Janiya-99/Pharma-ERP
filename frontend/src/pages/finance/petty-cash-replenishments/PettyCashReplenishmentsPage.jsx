...
export default function PettyCashReplenishmentsPage() {
  ...
  // Workflow Handlers
  const handleAction = async (actionFn, id, payload, modalName) => {
    ...
    <div className="flex flex-col gap-4 py-4 h-full">
      ...
      <div className="bg-white dark:bg-navy-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <select value={filters.petty_cash_fund_id} onChange={(e) => setFilters(prev => ({ ...prev, petty_cash_fund_id: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white">
            <option value="">All Funds</option>
            {funds.map(f => <option key={f.id} value={f.id}>{f.fund_code} - {f.fund_name}</option>)}
          </select>
          <select value={filters.bank_account_id} onChange={(e) => setFilters(prev => ({ ...prev, bank_account_id: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white">
            <option value="">All Bank Accounts</option>
            {bankAccounts.map(b => <option key={b.id} value={b.id}>{b.bank_name} - {b.account_number}</option>)}
          </select>
          <select value={filters.approval_status} onChange={(e) => setFilters(prev => ({ ...prev, approval_status: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white">
          ...
          <select value={filters.posted_status} onChange={(e) => setFilters(prev => ({ ...prev, posted_status: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white">
            <option value="">All Posted Statuses</option>
            <option value="unposted">Unposted</option>
            <option value="posted">Posted</option>
          </select>
          <input type="text" placeholder="Search Number or Ref..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white" />
        </div>
      </div>
      ...
      <div className="flex-1 bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-navy-700 text-gray-700 dark:text-gray-300">
              {loading ? (
                ...
                replenishments.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-navy-700/30">
                    <td className="px-4 py-3 font-medium text-navy-700 dark:text-white">{r.replenishment_number || '-'}</td>
                      ...
                      <div className="flex flex-col gap-2 justify-center items-center">
                        <div className="flex items-center gap-2">
                          {hasPermission("finance.petty_cash_replenishment.view") && (
                            <button onClick={() => history.push(`/admin/finance/petty-cash-replenishments/${r.id}`)} className="text-gray-500 hover:text-navy-700 dark:hover:text-white" title="View Details">
                        ...
                        <PettyCashActionButtons
                          ...
                          onSubmit={() => { setActionReplenishmentId(r.id); setModals(m => ({ ...m, submit: true })); }}
                          onApprove={() => { setActionReplenishmentId(r.id); setModals(m => ({ ...m, approve: true })); }}
                          onReject={() => { setActionReplenishmentId(r.id); setModals(m => ({ ...m, reject: true })); }}
                          onPost={() => { setActionReplenishmentId(r.id); setModals(m => ({ ...m, post: true })); }}
                        />
                      </div>
                    </td>
                  </tr>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-navy-700 bg-gray-50 dark:bg-navy-800 text-gray-500 dark:text-gray-400">
          ...
          <div className="flex gap-2">
            <button disabled={pagination.page <= 1} onClick={() => setPagination(prev => ({ ...prev, page: prev.page - 1 }))} className="px-3 py-1 border border-gray-300 dark:border-navy-600 rounded disabled:opacity-50 hover:bg-gray-100 dark:hover:bg-navy-700 transition-colors">Previous</button>
            <span className="px-3 py-1">Page {pagination.page}</span>
            <button disabled={replenishments.length < pagination.limit} onClick={() => setPagination(prev => ({ ...prev, page: prev.page + 1 }))} className="px-3 py-1 border border-gray-300 dark:border-navy-600 rounded disabled:opacity-50 hover:bg-gray-100 dark:hover:bg-navy-700 transition-colors">Next</button>
          </div>
        </div>
      </div>
      ...
      <SubmitPettyCashModal
        ...
        onSubmit={(payload) => handleAction(financeApi.submitPettyCashReplenishment, actionReplenishmentId, payload, "submit")}
      />
      <ApprovePettyCashModal
        isOpen={modals.approve}
        onClose={() => setModals(m => ({ ...m, approve: false }))}
        isSubmitting={actionLoading}
        onApprove={(payload) => handleAction(financeApi.approvePettyCashReplenishment, actionReplenishmentId, payload, "approve")}
      />
      <RejectPettyCashModal
        isOpen={modals.reject}
        onClose={() => setModals(m => ({ ...m, reject: false }))}
        isSubmitting={actionLoading}
        onReject={(payload) => handleAction(financeApi.rejectPettyCashReplenishment, actionReplenishmentId, payload, "reject")}
      />
      <PostPettyCashConfirmModal
        isOpen={modals.post}
        onClose={() => setModals(m => ({ ...m, post: false }))}
        isSubmitting={actionLoading}
