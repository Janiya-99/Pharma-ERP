...
export default function PettyCashReplenishmentDetailsPage() {
  ...
  return (
    <div className="flex flex-col gap-4 py-4 h-full max-w-5xl mx-auto">
          ...
          <div className="bg-white  rounded-xl shadow-sm border border-gray-100  p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
              ...
              <div>
                ...
                <span className="block mt-1 text-lg text-gray-900  font-bold"><MoneyDisplay amount={replenishment.replenishment_amount} /></span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Petty Cash Fund</span>
                <span className="block mt-1 text-base text-gray-900 ">{replenishment.petty_cash_fund?.fund_code} - {replenishment.petty_cash_fund?.fund_name}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Funding Bank Account</span>
                <span className="block mt-1 text-base text-gray-900 ">{replenishment.bank_account?.bank_name} - {replenishment.bank_account?.account_number}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Branch</span>
                <span className="block mt-1 text-base text-gray-900 ">{replenishment.branch?.branch_name}</span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500 ">Reference Number</span>
                <span className="block mt-1 text-base text-gray-900 ">{replenishment.reference_number || '-'}</span>
              </div>
              <div className="md:col-span-2">
                <span className="block text-sm font-medium text-gray-500 ">Remarks</span>
                <span className="block mt-1 text-base text-gray-900 ">{replenishment.remarks || '-'}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <SubmitPettyCashModal isOpen={modals.submit} onClose={() => setModals((m: unknown) => ({ ...m, submit: false }))} isSubmitting={actionLoading} onSubmit={(payload: Record<string, unknown>) => handleAction(financeApi.submitPettyCashReplenishment, payload, "submit")} />
      <ApprovePettyCashModal isOpen={modals.approve} onClose={() => setModals((m: unknown) => ({ ...m, approve: false }))} isSubmitting={actionLoading} onApprove={(payload: Record<string, unknown>) => handleAction(financeApi.approvePettyCashReplenishment, payload, "approve")} />
      <RejectPettyCashModal isOpen={modals.reject} onClose={() => setModals((m: unknown) => ({ ...m, reject: false }))} isSubmitting={actionLoading} onReject={(payload: Record<string, unknown>) => handleAction(financeApi.rejectPettyCashReplenishment, payload, "reject")} />
      <PostPettyCashConfirmModal isOpen={modals.post} onClose={() => setModals((m: unknown) => ({ ...m, post: false }))} isSubmitting={actionLoading} onConfirm={() => handleAction(financeApi.postPettyCashReplenishment, undefined, "post")} />
    </div>
  );
}
