    ...
    <div className="flex flex-col gap-4 py-4 h-full">
      ...
      <div className="bg-white dark:bg-navy-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <select value={filters.bank_account_id} onChange={(e: any) => setFilters((prev: unknown) => ({ ...prev, bank_account_id: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white">
          ...
          <input type="text" placeholder="Search Number or Ref..." value={filters.search} onChange={(e: any) => setFilters((prev: unknown) => ({ ...prev, search: e.target.value }))} className="w-full px-3 py-2 border rounded-md dark:bg-navy-900 dark:border-navy-600 dark:text-white" />
        </div>
      </div>
      ...
      <div className="flex-1 bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-navy-700 bg-gray-50 dark:bg-navy-800 text-gray-500 dark:text-gray-400">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={replenishments.length < pagination.limit} onClick={() => setPagination((prev: unknown) => ({ ...prev, page: prev.page + 1 }))} className="px-3 py-1 border border-gray-300 dark:border-navy-600 rounded disabled:opacity-50 hover:bg-gray-100 dark:hover:bg-navy-700 transition-colors">Next</button>
          </div>
        </div>
      </div>
