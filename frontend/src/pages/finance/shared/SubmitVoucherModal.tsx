...
export default function SubmitVoucherModal({
  isOpen,
  onClose,
  voucher,
  type,
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || `Failed to submit ${type} voucher`
      );
    } finally {
      setLoading(false);
    }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Submit ${displayType}`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <p className="text-sm text-gray-600">
          You are about to submit {displayType.toLowerCase()}{" "}
          <strong>{voucherNumber || "this voucher"}</strong> for approval.
        </p>
        <div>
