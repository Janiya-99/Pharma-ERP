...
const CashAccountsPage = () => {
  ...
  const fetchLookups = async () => {
    try {
      ...
      toast.error(
        getErrorMessage(error, "Failed to load cash account lookups")
      );
    }
  };
  ...
  const setField = <K extends keyof CashAccountForm>(
    key: K,
    value: CashAccountForm[K]
  ) => {
  ...
  const openEditDialog = (account: CashAccount) => {
    ...
    setForm({
      ...
      responsible_user_id: account.responsible_user_id
        ? String(account.responsible_user_id)
        : "",
      ...
      linked_ledger_account_id: account.linked_ledger_account_id
        ? String(account.linked_ledger_account_id)
        : "",
      auto_create_ledger: false,
  ...
  const validate = () => {
    ...
    if (!form.cash_account_name.trim())
      nextErrors.cash_account_name = "Cash Account Name is required.";
    ...
    if (!form.auto_create_ledger && !form.linked_ledger_account_id) {
      nextErrors.linked_ledger_account_id =
    ...
    try {
      const payload = {
        ...
        linked_ledger_account_id: form.linked_ledger_account_id
          ? Number(form.linked_ledger_account_id)
          : undefined,
    ...
    return (
      <div className="min-h-full bg-[#F8FAFC] p-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Please select a company before using Finance cash accounts.
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="text-slate-900 min-h-full bg-[#F8FAFC] p-6">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 max-w-3xl text-sm">
            Manage cash locations such as main cash, petty cash, branch cash,
            and cashier cash.
          </p>
        </div>
      ...
      <Card className="border-slate-200 mb-4 border bg-white shadow-sm">
        ...
        <CardHeader className="border-slate-100 border-b">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <CardDescription>
                Cash locations and their linked ledger accounts.
              </CardDescription>
            </div>
              ...
              {loading ? (
                ...
                accounts.map((account) => (
                  <TableRow key={account.id}>
                    ...
                    <TableCell>
                      {account.branch?.branch_name || account.branch_id || "-"}
                    </TableCell>
                    <TableCell>
                      {account.responsible_user?.name ||
                        account.responsible_user?.full_name ||
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            aria-label={`Actions for ${account.cash_account_name}`}
                          >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-white sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Cash accounts must link to a cash ledger account or request
              backend auto-create.
            </DialogDescription>
          </DialogHeader>
          {errors.company_id && (
          ...
          >
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <Field
                ...
                error={errors.cash_account_name}
                required
              >
                <Input
                  value={form.cash_account_name}
                  onChange={(event) =>
                    setField("cash_account_name", event.target.value)
                  }
                />
              </Field>
              ...
              <Field
                ...
                error={errors.linked_ledger_account_id}
              ...
              >
                <Select
                  ...
                  onValueChange={(value) =>
                    setField("linked_ledger_account_id", value)
                  }
                >
                  <SelectTrigger className="w-full">
              ...
              <div className="border-slate-200 bg-slate-50 flex items-center justify-between rounded-xl border p-4">
                <div>
                  ...
                  <p className="text-slate-500 mt-1 text-xs">
                    Backend should create Assets / Current Assets / Cash
                    Accounts ledger.
                  </p>
                </div>
                <Switch
                  checked={form.auto_create_ledger}
                  onCheckedChange={(checked) =>
                    setField("auto_create_ledger", checked)
                  }
                />
              </div>
            </div>
