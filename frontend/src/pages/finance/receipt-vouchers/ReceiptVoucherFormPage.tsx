...
export default function ReceiptVoucherFormPage() {
  ...
  const handleSave = async (e: any) => {
    ...
    if (
      ...
      setError(
        "Branch, Financial Year, Accounting Period, and Receipt Date are required."
      );
      return;
    }
    ...
    if (
      ...
      !formData.receipt_method ||
      !formData.received_to_account_id
    ) {
      setError(
        "Receipt Type, Receipt Method, and Received To Account are required."
      );
      return;
    }
    ...
    const payload = {
      ...
      cheque_number:
        formData.receipt_method === "cheque" ? formData.cheque_number : "",
      cheque_date:
        formData.receipt_method === "cheque" && formData.cheque_date
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Failed to save receipt voucher"
      );
      console.error(err);
    } finally {
      setSubmitting(false);
