...
export default function ReceiptVouchersPage() {
  ...
  const handleAction = async (action, voucher) => {
    if (action === "edit") {
      ...
      <div className="bg-white dark:bg-navy-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <FinancialYearSelect
            ...
            onChange={(val) => setFilters(prev => ({ ...prev, financial_year_id: val, accounting_period_id: "" }))}
            placeholder="All Financial Years"
          />
          <AccountingPeriodSelect
            financialYearId={filters.financial_year_id}
            value={filters.accounting_period_id}
            onChange={(val) => setFilters(prev => ({ ...prev, accounting_period_id: val }))}
            placeholder="All Accounting Periods"
          />
          <select
            value={filters.receipt_type}
            onChange={(e) => setFilters(prev => ({ ...prev, receipt_type: e.target.value }))}
          ...
          <select
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, receipt_method: e.target.value }))}
            className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-navy-500"
          >
          ...
          <select
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, posted_status: e.target.value }))}
            className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-navy-500"
          >
          ...
          <input
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, receipt_date_from: e.target.value }))}
            className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-navy-500"
          ...
          <input
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, receipt_date_to: e.target.value }))}
            className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-navy-500"
