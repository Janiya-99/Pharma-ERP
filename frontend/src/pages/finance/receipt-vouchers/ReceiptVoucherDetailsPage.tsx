...
export default function ReceiptVoucherDetailsPage() {
  ...
  if (error || !voucher) return <div className="p-8 text-center text-red-500">{error || "Receipt voucher not found"}</div>;
              ...
              <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 before:to-transparent">
                {voucher.approvals.map((approval: unknown, idx: unknown) => (
                  <div key={approval.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white bg-indigo-100 text-indigo-500 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow">
                      {idx + 1}
                    </div>
