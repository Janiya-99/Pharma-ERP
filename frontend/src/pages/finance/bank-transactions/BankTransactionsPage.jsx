  ...
  return (
    <div className="flex flex-col gap-4 py-4 h-full">
      <FinancePageHeader
        ...
        onAdd={hasPermission("finance.bank_transaction.create") ? () => history.push("/admin/finance/bank-transactions/create") : undefined}
      ...
      <div className="bg-white dark:bg-navy-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <BankAccountSelect
            ...
            onChange={(val) => setFilters(prev => ({ ...prev, bank_account_id: val }))}
      ...
      <div className="flex-1 bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden">
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-navy-700">
              {loading ? (
                ...
                transactions.map(tx => (
                  <tr key={tx.id} className="hover:bg-gray-50 dark:hover:bg-navy-700/30">
                    <td className="px-4 py-3 whitespace-nowrap">{new Date(tx.transaction_date).toLocaleDateString()}</td>
                    ...
                    <td className="px-4 py-3">{tx.bank_account?.bank_name} - {tx.bank_account?.account_number}</td>
                    ...
                    <td className="px-4 py-3"><BankTransactionAmountDisplay debitAmount={tx.debit_amount} creditAmount={0} /></td>
                    <td className="px-4 py-3"><BankTransactionAmountDisplay debitAmount={0} creditAmount={tx.credit_amount} /></td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${tx.is_reconciled ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {hasPermission("finance.bank_transaction.view") && (
                          <button onClick={() => history.push(`/admin/finance/bank-transactions/${tx.id}`)} className="text-gray-500 hover:text-navy-700">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-navy-700">
          ...
          <div className="flex gap-2">
            <button disabled={pagination.page <= 1} onClick={() => setPagination(prev => ({ ...prev, page: prev.page - 1 }))} className="px-3 py-1 border rounded disabled:opacity-50">Previous</button>
            <span className="px-3 py-1">Page {pagination.page}</span>
            <button disabled={transactions.length < pagination.limit} onClick={() => setPagination(prev => ({ ...prev, page: prev.page + 1 }))} className="px-3 py-1 border rounded disabled:opacity-50">Next</button>
          </div>
        </div>
      </div>
    </div>
