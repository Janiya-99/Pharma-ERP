  ...
  return (
    <div className="flex h-full flex-col gap-4 py-4">
      <FinancePageHeader
        ...
        onAdd={
          hasPermission("finance.petty_cash_fund.create")
            ? () => history.push("/admin/finance/petty-cash-funds/create")
            : undefined
        }
      ...
      <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-navy-700 dark:bg-navy-800">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <select
            ...
            className="w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-1 focus:ring-navy-500 dark:border-navy-700 dark:bg-navy-900 dark:text-white"
          >
          ...
          <select
            ...
            onChange={(e: any) =>
              setFilters((prev: unknown) => ({
                ...prev,
                custodian_user_id: e.target.value,
              }))
            }
            className="w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-1 focus:ring-navy-500 dark:border-navy-700 dark:bg-navy-900 dark:text-white"
          >
      ...
      <div className="flex-1 overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-navy-700 dark:bg-navy-800">
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700 dark:divide-navy-700 dark:text-gray-300">
              {loading ? (
                ...
                funds.map((fund: unknown) => {
                  const isLowBalance =
                    fund.current_balance < fund.fund_limit * 0.2;
                  return (
                    <tr
                      key={fund.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {fund.custodian_user?.name ||
                          fund.custodian_user?.full_name}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <MoneyDisplay amount={fund.opening_balance} />
                      </td>
                      <td
                      ...
                      <td className="px-4 py-3 text-right">
                        <MoneyDisplay amount={fund.fund_limit} />
                      </td>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          {hasPermission("finance.petty_cash_fund.view") && (
                            <button
                              onClick={() =>
                                history.push(
                                  `/admin/finance/petty-cash-funds/${fund.id}`
                                )
                              }
                          ...
                          {hasPermission("finance.petty_cash_fund.update") && (
                            <button
                              onClick={() =>
                                history.push(
                                  `/admin/finance/petty-cash-funds/${fund.id}/edit`
                                )
                              }
        ...
        <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50 px-4 py-3 text-gray-500 dark:border-navy-700 dark:bg-navy-800 dark:text-gray-400">
          ...
          <div className="flex gap-2">
            ...
            <button
              disabled={funds.length < pagination.limit}
              onClick={() =>
                setPagination((prev: unknown) => ({
                  ...prev,
                  page: prev.page + 1,
