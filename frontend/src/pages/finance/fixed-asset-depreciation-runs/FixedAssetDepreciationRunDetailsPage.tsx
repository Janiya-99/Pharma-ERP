        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-navy-700 ">{run.run_number}</h2>
                  <p className="text-sm font-medium text-gray-500  mt-1">Run Date: {new Date(run.run_date).toLocaleDateString()}</p>
                </div>
                <DepreciationPostedStatusBadge status={run.posted_status} />
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-8 mt-6 pt-6 border-t border-gray-100 ">
                <div className="flex flex-col">
                  <span className="text-xs text-gray-500 mb-1">Financial Year</span>
                  <span className="text-sm font-medium text-navy-700 ">{run.financial_year?.year_name}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-gray-500 mb-1">Accounting Period</span>
                  <span className="text-sm font-medium text-navy-700 ">{run.accounting_period?.period_name}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-gray-500 mb-1">Branch Scope</span>
                  <span className="text-sm font-medium text-navy-700 ">{run.branch?.branch_name || "All Branches"}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-gray-500 mb-1">From Date</span>
                  <span className="text-sm font-medium text-navy-700 ">{new Date(run.depreciation_from_date).toLocaleDateString()}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-gray-500 mb-1">To Date</span>
                  <span className="text-sm font-medium text-navy-700 ">{new Date(run.depreciation_to_date).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              {run.remarks && (
                <div className="mt-6 pt-4 border-t border-gray-100 ">
                  ...
                  <p className="text-sm text-gray-700 ">{run.remarks}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-gradient-to-br from-brand-50 to-white   rounded-2xl shadow-sm border border-brand-100  p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-brand-100  pb-3">
                  <span className="text-sm text-gray-600 ">Total Assets Depreciated</span>
                ...
                <div className="flex justify-between items-center pt-2">
                  <span className="text-sm font-semibold text-brand-700 ">Total Depreciation</span>
            ...
            <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs font-medium text-navy-700 ">{run.created_by_user?.name || "-"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-gray-500">Created At</span>
                  <span className="text-xs font-medium text-navy-700 ">{new Date(run.created_at).toLocaleString()}</span>
                </div>
                {run.posted_status === "posted" && (
                  <>
                    <div className="flex justify-between mt-2 pt-2 border-t border-gray-100 ">
                      <span className="text-xs text-gray-500">Posted By</span>
                      <span className="text-xs font-medium text-navy-700 ">{run.posted_by_user?.name || "-"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-xs text-gray-500">Posted At</span>
                      <span className="text-xs font-medium text-navy-700 ">{new Date(run.posted_at).toLocaleString()}</span>
                    </div>
                  </>
                )}
              </div>
