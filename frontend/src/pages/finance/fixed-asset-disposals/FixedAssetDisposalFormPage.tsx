...
const FixedAssetDisposalFormPage = () => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.fixed_asset_id)
      newErrors.fixed_asset_id = "Asset is required";
    if (!formData.disposal_date)
      newErrors.disposal_date = "Disposal Date is required";
    if (!formData.disposal_type)
      newErrors.disposal_type = "Disposal Type is required";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        sale_value:
          formData.disposal_type === "sale" ? Number(formData.sale_value) : 0,
      ...
      toast.error(
        err.response?.data?.message || "Failed to save asset disposal"
      );
    } finally {
      setSaving(false);
    }
  ...
  const nbv = selectedAssetDetails
    ? Number(selectedAssetDetails.net_book_value || 0)
    : 0;
  const saleVal =
    formData.disposal_type === "sale" ? Number(formData.sale_value || 0) : 0;
                ...
                {formData.disposal_type === "sale" && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e: any) =>
                        setFormData({ ...formData, sale_value: e.target.value })
                      }
                    />
                    {errors.sale_value && (
                      <span className="mt-1 text-xs text-red-500">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs font-medium text-navy-700 dark:text-white">
                      LKR{" "}
                      {Number(
                        selectedAssetDetails.acquisition_cost || 0
                      ).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs font-medium text-navy-700 dark:text-white">
                      LKR{" "}
                      {Number(
                        selectedAssetDetails.accumulated_depreciation || 0
                      ).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between border-t border-gray-50 pt-2 dark:border-navy-700/50">
                    ...
                    <span className="max-w-[150px] truncate text-xs font-medium text-navy-700 dark:text-white">
                      {selectedAssetDetails.gain_on_disposal_account_id
                        ? "Configured"
                        : "Missing"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="max-w-[150px] truncate text-xs font-medium text-navy-700 dark:text-white">
                      {selectedAssetDetails.loss_on_disposal_account_id
                        ? "Configured"
                        : "Missing"}
                    </span>
                  </div>
