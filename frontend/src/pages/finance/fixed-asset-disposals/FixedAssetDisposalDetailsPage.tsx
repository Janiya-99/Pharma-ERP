    ...
    <div className="flex flex-col h-full bg-gray-50  overflow-y-auto pb-10">
      ...
      <div className="px-6 max-w-6xl mx-auto w-full">
        ...
        {disposal.status === "rejected" && disposal.approved_rejected_remarks && (
          <div className="mb-6 p-4 bg-red-50  rounded-xl border border-red-200 ">
            ...
            <p className="text-sm text-red-700 ">{disposal.approved_rejected_remarks}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-navy-700 ">{disposal.disposal_number}</h2>
                  <p className="text-sm font-medium text-gray-500  mt-1">Disposal Date: {new Date(disposal.disposal_date).toLocaleDateString()}</p>
                </div>
                {/* Custom Status Badge */}
              ...
              <div className="bg-gray-50  rounded-xl p-4 mb-6 border border-gray-100 ">
                <h3 className="text-sm font-bold text-navy-700  mb-3">Asset Information</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-xs text-gray-500 block">Asset Code</span>
                    <span className="text-sm font-medium text-navy-700 ">{disposal.fixed_asset?.asset_code}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500 block">Asset Name</span>
                    <span className="text-sm font-medium text-navy-700 ">{disposal.fixed_asset?.asset_name}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500 block">Category</span>
                    <span className="text-sm font-medium text-navy-700 ">{disposal.fixed_asset?.category?.category_name}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500 block">Branch</span>
                    <span className="text-sm font-medium text-navy-700 ">{disposal.fixed_asset?.branch?.branch_name}</span>
                  </div>
                </div>
              </div>
              ...
              {disposal.remarks && (
                <div className="mt-6 pt-4 border-t border-gray-100 ">
                  ...
                  <p className="text-sm text-gray-700 ">{disposal.remarks}</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center pt-2">
                  ...
                  <div className="flex flex-col items-end">
                    <span className={`text-xl font-bold ${disposal.gain_loss_amount > 0 ? "text-green-600 " : disposal.gain_loss_amount < 0 ? "text-red-600 " : "text-gray-600 "}`}>
            ...
            <div className="bg-white  rounded-2xl shadow-sm border border-gray-100  p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs font-medium text-navy-700 ">{disposal.created_by_user?.name || "-"}</span>
                </div>
                {(disposal.status === "approved" || disposal.status === "rejected" || disposal.status === "posted") && (
                  <div className="flex justify-between pt-2 border-t border-gray-50 ">
                    ...
                    <span className="text-xs font-medium text-navy-700 ">{disposal.approved_rejected_by_user?.name || "-"}</span>
                  </div>
                )}
                {disposal.status === "posted" && (
                  <div className="flex justify-between pt-2 border-t border-gray-50 ">
                    <span className="text-xs text-gray-500">Posted By</span>
                    <span className="text-xs font-medium text-navy-700 ">{disposal.posted_by_user?.name || "-"}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <Modal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Delete Asset Disposal">
        <p className="text-gray-700  mb-6">
          Are you sure you want to delete disposal <span className="font-bold text-navy-700 ">{disposal.disposal_number}</span>?
        </p>
