...
export default function PaymentVoucherDetailsPage() {
  ...
  const handleAction = async (action: unknown, vch: unknown) => {
    if (action === "edit") {
      ...
      if (
        window.confirm("Are you sure you want to delete this payment voucher?")
      ) {
        try {
        ...
        <div className="space-y-4 md:col-span-2">
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm dark:border-navy-700 dark:bg-navy-800">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="mb-1 text-gray-500">Paid From Account</p>
                <p className="font-medium text-navy-800 dark:text-white">
                  {voucher.paid_from_account?.account_code} -{" "}
                  {voucher.paid_from_account?.account_name}
                </p>
              </div>
