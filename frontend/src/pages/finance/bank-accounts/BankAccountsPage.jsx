  ...
  return (
    <div className="flex flex-col gap-4 py-4 h-full">
      <FinancePageHeader
        ...
        onAdd={hasPermission("finance.bank_account.create") ? () => history.push("/admin/finance/bank-accounts/create") : undefined}
      ...
      <div className="bg-white dark:bg-navy-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <select
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, is_default: e.target.value }))}
            className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-navy-500"
          >
      ...
      <div className="flex-1 bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden">
              ...
              {loading ? (
                ...
                accounts.map(account => (
                  <tr key={account.id} className="hover:bg-gray-50 dark:hover:bg-navy-700/30">
                    ...
                    <td className="px-4 py-3">{account.chart_of_account?.account_code} - {account.chart_of_account?.account_name}</td>
                    <td className="px-4 py-3 text-right font-medium"><MoneyDisplay amount={account.current_balance} /></td>
                    <td className="px-4 py-3 text-center space-y-1">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${account.status === "active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
                        {account.status}
                      </span>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {hasPermission("finance.bank_account.view") && (
                          <button onClick={() => history.push(`/admin/finance/bank-accounts/${account.id}`)} className="text-gray-500 hover:text-navy-700">
                            <MdVisibility className="h-5 w-5" />
                          </button>
                        )}
                        {hasPermission("finance.bank_account.update") && (
                          <button onClick={() => history.push(`/admin/finance/bank-accounts/${account.id}/edit`)} className="text-brand-500 hover:text-brand-700">
                            <MdEdit className="h-5 w-5" />
                          </button>
                        )}
                        {hasPermission("finance.bank_account.delete") && (
                          <button onClick={() => handleDelete(account.id)} className="text-red-500 hover:text-red-700">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-navy-700">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={accounts.length < pagination.limit} onClick={() => setPagination(prev => ({ ...prev, page: prev.page + 1 }))} className="px-3 py-1 border rounded disabled:opacity-50">Next</button>
          </div>
        </div>
      </div>
    </div>
