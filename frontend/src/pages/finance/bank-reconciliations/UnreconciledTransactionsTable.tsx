...
export default function UnreconciledTransactionsTable({
  transactions,
  selectedTxIds,
  onToggleSelection,
  onToggleAll,
  ...
  selectedTxIds?: unknown;
  onToggleSelection?: unknown;
  onToggleAll?: unknown;
}) {
  const allSelected =
    transactions.length > 0 && selectedTxIds.length === transactions.length;
            ...
            {transactions.length === 0 ? (
              <tr>
                <td colSpan="6" className="px-4 py-8 text-center text-gray-500">
                  No unreconciled transactions available.
                </td>
              </tr>
            ) : (
              transactions.map((tx: unknown) => (
                <tr
                  key={tx.id}
                  className={`cursor-pointer hover:bg-gray-50 dark:hover:bg-navy-700/30 ${
