...
export default function BankReconciliationFormPage() {
  ...
  const fetchFormData = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        if (rec.lines) {
          setSelectedTxIds(rec.lines.map(line => line.bank_transaction_id));
        }
  ...
  const fetchUnreconciledTxs = async (bankAccountId, existingLines = []) => {
    try {
      ...
      if (res.data?.success) {
        ...
        if (isEdit && existingLines.length > 0) {
          const currentTxs = existingLines.map(line => line.bank_transaction);
          // Filter out existing from fetched to avoid duplicates, then prepend
          const fetchedIds = new Set(txs.map(t => t.id));
          const missingCurrent = currentTxs.filter(ct => ct && !fetchedIds.has(ct.id));
  ...
  const toggleAll = (e) => {
    if (e.target.checked) {
      setSelectedTxIds(unreconciledTxs.map(tx => tx.id));
    } else {
      setSelectedTxIds([]);
    }
  };
  ...
  const calculateSummary = () => {
    ...
    selectedTxIds.forEach(id => {
      const tx = unreconciledTxs.find(t => t.id === id);
      if (tx) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  ...
                  <BankAccountSelect
                    ...
                    onChange={(val) => setFormData(prev => ({ ...prev, bank_account_id: val }))}
                    disabled={isEdit}
                  />
                </div>
