...
function CardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "group/card-header @container/card-header has-data-[slot=card-action]:grid-cols-[1fr_auto] has-data-[slot=card-description]:grid-rows-[auto_auto] [.border-b]:pb-[var(--card-spacing)] grid auto-rows-min items-start gap-1 rounded-t-xl px-[var(--card-spacing)]",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "bg-muted/50 flex items-center rounded-b-xl border-t p-[var(--card-spacing)]",
        className
      )}
      {...props}
    />
