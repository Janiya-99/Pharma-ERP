...
import { SalesInvoicePostedStatus } from '../../types/invoice-center';

interface Props {
  status: SalesInvoicePostedStatus;
  className?: string;
}

export const SalesInvoicePostedStatusBadge: React.FC<Props> = ({ status, className }) => {
  const getBadgeColor = () => {
    switch (status) {
