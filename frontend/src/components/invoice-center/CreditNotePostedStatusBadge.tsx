...
import { CreditNotePostedStatus } from '../../types/invoice-center';

interface Props {
  status: CreditNotePostedStatus;
  className?: string;
}

export const CreditNotePostedStatusBadge: React.FC<Props> = ({ status, className }) => {
  const getBadgeColor = () => {
    switch (status) {
