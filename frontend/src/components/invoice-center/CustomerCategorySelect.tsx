...
const CustomerCategorySelect: React.FC<CustomerCategorySelectProps> = ({ value, onChange, disabled = false, error }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await invoiceCenterApi.getCustomerCategories({ limit: 200, status: "active" });
  ...
  return (
    <div>
      ...
      >
        <option value="">{loading ? "Loading categories..." : "Select Customer Category"}</option>
        {categories.map((cat) => (
          <option key={cat.id} value={cat.id}>
            {cat.category_code} - {cat.category_name} (Limit: LKR {Number(cat.credit_limit || 0).toLocaleString()})
          </option>
        ))}
      </select>
