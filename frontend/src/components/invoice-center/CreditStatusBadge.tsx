...
const CreditStatusBadge: React.FC<CreditStatusBadgeProps> = ({
  ...
  if (balanceNum > limitNum) {
    return (
      <span className="bg-rose-50 text-rose-700 border-rose-200    inline-flex animate-pulse items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs font-semibold">
  ...
  return (
    <span className="bg-emerald-50 text-emerald-700 border-emerald-200    inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold">
      OK
    </span>
  );
};
