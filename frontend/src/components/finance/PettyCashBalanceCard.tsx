...
const PettyCashBalanceCard = ({ fund }: { fund?: unknown }) => {
  ...
  return (
    <div className="mb-6 grid grid-cols-1 gap-5 md:grid-cols-4 lg:grid-cols-4 2xl:grid-cols-4">
      ...
      <Card
        extra={`!flex-row flex-grow items-center rounded-xl p-4 ${
          isLowBalance ? "border-2 border-red-500" : ""
        }`}
      >
        <div
          className={`flex h-14 w-14 items-center justify-center rounded-full ${
            isLowBalance
              ...
              : "bg-green-50 dark:bg-navy-700"
          }`}
        >
          <span
            className={`text-xl ${
              isLowBalance ? "text-red-500" : "text-green-500 dark:text-white"
            }`}
          >
            CB
          </span>
