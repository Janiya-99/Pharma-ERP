...
const DepreciationLinesTable = ({ lines, loading }: { lines?: unknown; loading?: unknown }) => {
  const columns = [
    {
      key: "asset_code",
      label: "Asset Code",
    ...
    },
    {
      key: "asset_name",
      label: "Asset Name",
      render: (val: unknown, row: unknown) => <span>{row.fixed_asset?.asset_name || val}</span>,
    },
    {
    ...
    },
    {
      ...
      label: "Acc. After",
      align: "right",
      render: (val: unknown) => <span className="font-medium">LKR {Number(val || 0).toLocaleString()}</span>,
    },
    {
      key: "nbv_before",
      label: "NBV Before",
