...
const MultiSelect = ({ 
  ...
  const handleSelect = (option) => {
    const isSelected = selectedValues.some((v) => v[valueKey] === option[valueKey]);
    if (isSelected) {
      onChange(selectedValues.filter((v) => v[valueKey] !== option[valueKey]));
    } else {
        ...
        {selectedValues.map((selected) => (
          ...
          >
            ...
            <button
              ...
              onClick={(e) => handleRemove(e, selected)}
            >
              <X className="h-3 w-3" />
            </button>
          </span>
