...
const DataTable = ({ columns, data, loading, emptyTitle = "No records found", emptyDescription = "" }: { columns?: unknown; data?: Record<string, unknown>; loading?: unknown; emptyTitle?: unknown; emptyDescription?: unknown }) => {
  if (loading) {
    return (
    ...
    <div className="overflow-x-auto bg-white rounded-lg border border-gray-200 shadow-sm">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            {columns.map((col: unknown, index: unknown) => (
              <th
                key={index}
                scope="col"
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row: unknown, rowIndex: unknown) => (
            <tr key={rowIndex} className="hover:bg-gray-50 transition-colors">
              {columns.map((col: unknown, colIndex: unknown) => (
                <td
                  key={colIndex}
