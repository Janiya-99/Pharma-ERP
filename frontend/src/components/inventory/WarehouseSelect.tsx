...
const WarehouseSelect = ({ value, onChange, placeholder = "Select...", disabled = false, error = false, isMulti = false, isClearable = true, extraParams = {} }: { value?: unknown; onChange?: unknown; placeholder?: unknown; disabled?: unknown; error?: unknown; isMulti?: boolean; isClearable?: boolean; extraParams?: unknown }) => {
  ...
  const fetchOptions = async () => {
    setLoading(true);
    try {
      const response = await inventoryApi.getWarehouses({ limit: 1000, status: "active", ...extraParams });
      ...
      console.error("Failed to load WarehouseSelect options:", error);
    } finally {
      setLoading(false);
    }
  };
