...
const GRNTotalsCard = ({ grn, lines }) => {
  const tQty = lines?.reduce((sum, l) => sum + (Number(l.quantity_received) || 0), 0) || grn?.total_quantity || 0;
  const tFreeQty = lines?.reduce((sum, l) => sum + (Number(l.free_quantity) || 0), 0) || grn?.total_free_quantity || 0;
  const tStockQty = lines?.reduce((sum, l) => sum + (Number(l.quantity_received) || 0) + (Number(l.free_quantity) || 0), 0) || grn?.total_stock_quantity || 0;
  
  const subTotal = lines?.reduce((sum, l) => sum + ((Number(l.quantity_received) || 0) * (Number(l.unit_cost) || 0)), 0) || grn?.subtotal_amount || 0;
  const tDiscount = lines?.reduce((sum, l) => sum + (Number(l.discount_amount) || 0), 0) || grn?.discount_amount || 0;
  const tTax = lines?.reduce((sum, l) => sum + (Number(l.tax_amount) || 0), 0) || grn?.tax_amount || 0;
  const tTotal = lines?.reduce((sum, l) => sum + (((Number(l.quantity_received) || 0) * (Number(l.unit_cost) || 0)) - (Number(l.discount_amount) || 0) + (Number(l.tax_amount) || 0)), 0) || grn?.total_amount || 0;
