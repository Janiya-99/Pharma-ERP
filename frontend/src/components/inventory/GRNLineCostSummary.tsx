...
const GRNLineCostSummary = ({
  ...
  return (
    <div className="bg-slate-50 border-slate-100 flex min-w-[150px] flex-col gap-1 rounded border p-2 text-[11px] dark:border-navy-700 dark:bg-navy-800">
      ...
      <div className="border-slate-200 my-0.5 border-t dark:border-navy-600" />
      <div className="flex justify-between font-bold text-brand-600">
        <span>Line Total:</span>
        <span>{formatLKR(lineTotal)}</span>
      </div>
      <div className="border-slate-200 mt-0.5 flex justify-between border-t pt-0.5 dark:border-navy-600">
        <span className="text-slate-500">Stock U.Cost:</span>
        <span
          className={`font-bold ${
            stockUnitCost === 0 ? "text-red-500" : "text-green-600"
          }`}
        >
