...
const GRNTotalsCard = ({ grn, lines }: { grn?: unknown; lines?: unknown }) => {
  ...
  return (
    <div className="bg-gray-50  rounded-xl p-4 border border-gray-100 ">
      <h3 className="font-semibold text-gray-800  mb-3 text-sm border-b pb-2 ">GRN Totals</h3>
      
      <div className="space-y-2 text-sm">
        <div className="flex justify-between items-center">
          <span className="text-gray-500 ">Total Quantity:</span>
          <span className="font-medium text-gray-800 ">{Number(tQty).toFixed(3)}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-gray-500 ">Total Free Quantity:</span>
          <span className="font-medium text-gray-800 ">{Number(tFreeQty).toFixed(3)}</span>
        </div>
        <div className="flex justify-between items-center font-bold">
          <span className="text-brand-600 ">Total Stock Qty:</span>
          <span className="text-brand-600 ">{Number(tStockQty).toFixed(3)}</span>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="font-medium text-gray-800 ">{formatLKR(tTax)}</span>
        </div>
        ...
        <div className="flex justify-between items-center font-bold pt-2 border-t border-gray-200  text-base">
          ...
          <span className="text-brand-600 ">{formatLKR(tTotal)}</span>
        </div>
      </div>
    </div>
  );
