...
const AvailableStockCard = ({ stockBalance, loading }: { stockBalance?: unknown; loading?: unknown }) => {
  if (loading) {
    return (
  ...
  if (!stockBalance) {
    return (
      <div className="bg-gray-50  border border-dashed border-gray-300  rounded-lg p-3 flex flex-col items-center justify-center text-center">
  ...
  return (
    <div className="bg-white  border border-gray-200  rounded-lg overflow-hidden shadow-sm">
      <div className="px-3 py-2 bg-gray-50  border-b border-gray-200  flex justify-between items-center">
        <span className="text-xs font-semibold text-gray-600 ">Available Stock</span>
      ...
      <div className="p-3 grid grid-cols-2 gap-y-3 gap-x-4">
        <div>
          ...
          <span className="text-sm font-medium text-gray-900 ">{formatNumber(stockBalance.quantity_on_hand, 3)}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="text-sm font-medium text-gray-900 ">{formatCurrency(stockBalance.average_cost)}</span>
        </div>
      </div>
    </div>
  );
