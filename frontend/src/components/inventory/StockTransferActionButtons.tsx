...
const StockTransferActionButtons = ({
  ...
  onReject,
  onPost,
}: { approvalStatus?: unknown; postedStatus?: unknown; onEdit?: unknown; onDelete?: unknown; onSubmit?: unknown; onApprove?: unknown; onReject?: unknown; onPost?: unknown }) => {
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {!isPosted && isDraftOrRejected && onSubmit && (
        <PermissionGuard permission="inventory.stock_transfer.submit">
          <button
            onClick={onSubmit}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-brand-700 bg-brand-50 border border-brand-200 rounded-lg hover:bg-brand-100    "
          >
      ...
      {isApprovedAndUnposted && onPost && (
        <PermissionGuard permission="inventory.stock_transfer.post">
          <button
            onClick={onPost}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 focus:ring-4 focus:ring-green-100 "
          >
