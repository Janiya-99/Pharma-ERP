...
const DosageFormSelect = ({ value, onChange, placeholder = "Select...", disabled = false, error = false, isMulti = false, isClearable = true, extraParams = {} }: { value?: unknown; onChange?: unknown; placeholder?: unknown; disabled?: unknown; error?: unknown; isMulti?: boolean; isClearable?: boolean; extraParams?: unknown }) => {
  ...
  const fetchOptions = async () => {
    setLoading(true);
    try {
      const response = await inventoryApi.getDosageForms({ limit: 1000, status: "active", ...extraParams });
      if (response.data?.success) {
        const data = response.data.data;
        const mappedOptions = data.map((item: unknown) => ({
          value: item.id,
          label: `${item.dosage_form_code} - ${item.dosage_form_name}`,
          original: item,
        }));
        setOptions(mappedOptions);
      }
    } catch (error) {
      console.error("Failed to load DosageFormSelect options:", error);
    } finally {
      setLoading(false);
    }
  };
