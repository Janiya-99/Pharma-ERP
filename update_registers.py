...
files = {
    "frontend/src/pages/finance/reports/journal-register/JournalRegisterReportPage.jsx": """import React, { useState, useEffect, useCallback } from 'react';
  ...
  if (!hasPermission("finance.report.journal_register.view")) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400">You do not have permission to view the Journal Register report.</p>
      </div>
    );
  }
  ...
  const columns = [
    {
      ...
      cell: (row) => <div className="font-medium text-navy-800 dark:text-white">{row.journal_number}</div>
    },
    {
      header: "Date",
      accessor: "journal_date",
      cell: (row) => format(new Date(row.journal_date), 'yyyy-MM-dd')
    },
    {
      header: "Ref No",
    ...
    },
    {
      ...
      cell: (row) => <div className="max-w-[150px] truncate" title={row.description}>{row.description || '-'}</div>
    },
    {
      header: <div className="text-right">Total Debit</div>,
      accessor: "total_debit",
      cell: (row) => <ReportAmountCell amount={row.total_debit} />
    },
    {
      header: <div className="text-right">Total Credit</div>,
      accessor: "total_credit",
      cell: (row) => <ReportAmountCell amount={row.total_credit} />
    },
    {
    ...
    },
    {
      ...
      cell: (row) => <StatusBadge status={row.posted_status} />
    },
    {
      header: "Created By",
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val) => handleFilterChange('approval_status', val)}
          >
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val) => handleFilterChange('posted_status', val)}
          >
            <SelectTrigger>
              <SelectValue placeholder="All Statuses" />
            </SelectTrigger>
...
""",
    ...
    "frontend/src/pages/finance/reports/payment-register/PaymentRegisterReportPage.jsx": """import React, { useState, useEffect, useCallback } from 'react';
import { useAuth } from '../../../../auth/AuthContext';
import { financeApi } from '../../../../api/financeApi';
import { toast } from 'react-hot-toast';
import { format } from 'date-fns';
  ...
  const columns = [
    {
      ...
      cell: (row) => <div className="font-medium text-navy-800 dark:text-white">{row.voucher_number}</div>
    },
    {
      header: "Date",
      accessor: "payment_date",
      cell: (row) => format(new Date(row.payment_date), 'yyyy-MM-dd')
    },
    {
      header: "Type",
      accessor: "payment_type",
      cell: (row) => row.payment_type.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')
    },
    {
      header: "Method",
      accessor: "payment_method",
      cell: (row) => row.payment_method.charAt(0).toUpperCase() + row.payment_method.slice(1)
    },
    {
      header: "Paid From",
      accessor: "paid_from_account_name",
      cell: (row) => <div className="max-w-[150px] truncate" title={row.paid_from_account_name}>{row.paid_from_account_name || '-'}</div>
    },
    {
      header: "Ref No",
      accessor: "reference_number"
    },
    {
      ...
      cell: (row) => <ReportAmountCell amount={row.total_amount} />
    },
    {
      header: "Approval",
      accessor: "approval_status",
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val) => handleFilterChange('payment_type', val)}
          >
            <SelectTrigger>
...
""",
    ...
    "frontend/src/pages/finance/reports/receipt-register/ReceiptRegisterReportPage.jsx": """import React, { useState, useEffect, useCallback } from 'react';
import { useAuth } from '../../../../auth/AuthContext';
import { financeApi } from '../../../../api/financeApi';
import { toast } from 'react-hot-toast';
import { format } from 'date-fns';
  ...
  const columns = [
    {
      ...
      cell: (row) => <div className="font-medium text-navy-800 dark:text-white">{row.receipt_number}</div>
    },
    {
      header: "Date",
      accessor: "receipt_date",
      cell: (row) => format(new Date(row.receipt_date), 'yyyy-MM-dd')
    },
    {
      header: "Type",
      accessor: "receipt_type",
      cell: (row) => row.receipt_type.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')
    },
    {
      header: "Method",
      accessor: "receipt_method",
      cell: (row) => row.receipt_method.charAt(0).toUpperCase() + row.receipt_method.slice(1)
    },
    {
      header: "Received To",
      accessor: "received_to_account_name",
      cell: (row) => <div className="max-w-[150px] truncate" title={row.received_to_account_name}>{row.received_to_account_name || '-'}</div>
    },
    {
      header: "Ref No",
      accessor: "reference_number"
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val) => handleFilterChange('receipt_method', val)}
          >
            <SelectTrigger>
              <SelectValue placeholder="All Methods" />
            </SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val) => handleFilterChange('receipt_type', val)}
          >
            <SelectTrigger>
              <SelectValue placeholder="All Types" />
            </SelectTrigger>
...
print("Updated Register components.")
