...
type GenericNameHandler struct {
	service *services.GenericNameService
	logger  *zap.Logger
}

func NewGenericNameHandler(s *services.GenericNameService, logger *zap.Logger) *GenericNameHandler {
	return &GenericNameHandler{service: s, logger: logger}
}
func (h *GenericNameHandler) List(c *gin.Context) {
...
func (h *GenericNameHandler) Get(c *gin.Context) {
	companyID, _ := c.Get("company_id")
	companyDB, _ := c.Get("companyDB")
	db := companyDB.(*gorm.DB)
...
func (h *GenericNameHandler) Create(c *gin.Context) {
	companyID, _ := c.Get("company_id")
	companyDB, _ := c.Get("companyDB")
	db := companyDB.(*gorm.DB)
	var req dto.CreateGenericNameRequest
...
func (h *GenericNameHandler) Update(c *gin.Context) {
	companyID, _ := c.Get("company_id")
	companyDB, _ := c.Get("companyDB")
	db := companyDB.(*gorm.DB)
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
	var req dto.UpdateGenericNameRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": err.Error()})
		return
	}
...
func (h *GenericNameHandler) Delete(c *gin.Context) {
	companyID, _ := c.Get("company_id")
	companyDB, _ := c.Get("companyDB")
	db := companyDB.(*gorm.DB)
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
