...
// SeedSoftwareCatalog inserts the default software modules into the software_catalog table.
// This is safe to run multiple times — it only inserts records that do not already exist
// (matched by software_code).
func SeedSoftwareCatalog(db *gorm.DB, logger *zap.Logger) error {
	modules := []models.SoftwareCatalog{
		{
	...
	for _, module := range modules {
		...
		if result.Error == gorm.ErrRecordNotFound {
			...
			logger.Debug("Software module already exists, skipping",
				zap.String("code", module.SoftwareCode),
			)
		}
	}
